...
describe("what an import refuses", () => {
  ...
  it("refuses existing device trust and sessions before importing any identity or key", async () => {
    const installationId = generateUuidV7();
    ...
    await expect(service().import(makeKit())).rejects.toThrow(/not empty/i);
    expect(await handle.db.select().from(schema.authorizedDevices)).toEqual(devices);
    expect(await handle.db.select().from(schema.sessions)).toEqual(sessions);
    expect(await handle.db.select().from(schema.installations)).toEqual(installations);
    expect(await handle.db.select().from(schema.workspaceRootKeys)).toHaveLength(0);
    expect(await handle.db.select().from(schema.wrappingKeyVersions)).toHaveLength(0);
  });
