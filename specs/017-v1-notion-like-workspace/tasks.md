...
- [X] T004 Configurer Tailwind CSS via Vite et l'entrée CSS commune dans `apps/web/vite.config.ts`, `apps/web/src/global.css` et `apps/web/package.json`
...
- [X] T042 Définir tokens clair/sombre, couleurs de contenu, focus, espace, couches et mouvement dans `apps/web/src/ui/tokens.css` et `apps/web/src/global.css`
...
- [x] T146 [US5] Consolider après 30 s idle, 5 min max et bornes, exposer le retard de projection et restaurer par opérations dans `apps/api/src/page-state/page-history-service.ts` et `apps/api/src/routes/revisions.ts`
- [x] T147 [US5] Étendre archives, verify et restore aux états opérationnels dans `apps/api/src/backup/archive-format.ts`, `apps/api/src/backup/backup-service.ts` et `apps/api/src/backup/restore-service.ts`
...
- [X] T165 [US7] Garantir cibles tactiles et alternatives au hover/clic droit dans `apps/web/src/ui/tokens.css` et `apps/web/src/global.css`
- [X] T166 [US7] Corriger overflow 320 px/200 %, viewport clavier virtuel et popovers aux bords dans `apps/web/src/global.css` et `apps/web/src/ui/primitives/popover.tsx`
- [X] T167 [US7] Vérifier reduced-motion et statuts visibles non intrusifs save/sync dans `apps/web/src/ui/tokens.css`, `apps/web/src/global.css` et `tests/e2e/touch-and-motion.spec.ts`
...
- [X] T185 [US6] Supprimer variantes CSS et ancien composant sync sans consommateur dans `apps/web/src/global.css` et `apps/web/src/components/sync-status.tsx`
...
- [X] T216 [US5] Borner le replay serveur par des checkpoints Loro complets périodiques sans compacter l'historique, importer les lots causalement, paginer la fenêtre d'ambiguïté sans plafond de 10 000, avancer chaque frontière d'appareil depuis son préfixe confirmé, ne pas renvoyer à un appareil une update qu'il possède déjà et indexer les références d'enveloppes retirées par la compaction dans `apps/api/src/page-state/page-operation-service.ts`, `packages/page-state/src/document.ts`, `packages/database/src/schema/page-operations.ts`, `packages/database/migrations/0011_page_operation_compaction_indexes.sql`, `packages/page-state/tests/checkpoints.property.spec.ts` et `apps/api/tests/page-operation-long-absence.integration.spec.ts` per FR-054, FR-058, FR-059 et SC-019
- [X] T217 [US5] Reconstruire sans perte la branche récupérable lorsqu'un appareil insère des descendants sous un parent supprimé concurremment, y compris plusieurs niveaux, leur ordre et les cellules de tableau, dans `packages/page-state/src/semantic-conflicts.ts` et `packages/page-state/tests/semantic-conflicts.property.spec.ts` per FR-058 et SC-018
- [X] T218 [US5] Séparer l'intégrité binaire d'un checkpoint de l'identité causale stable de son état, puis prouver le rollover après 512 updates et sa réouverture dans `packages/page-state/src/update-envelope.ts`, `packages/page-state/src/canonical-projection.ts`, `apps/api/src/page-state/page-operation-service.ts`, `packages/client-core/src/page-sync/encrypted-update-log.ts` et `packages/page-state/tests/checkpoints.property.spec.ts` per FR-054, FR-058 et SC-019
- [X] T219 [US5] Attendre l'évaluation de sécurité initiale avant de déclarer l'API prête, arrêter son ordonnanceur avec Fastify et verrouiller la frontière par un test PostgreSQL déterministe dans `apps/api/src/app.ts` et `apps/api/tests/production-startup.spec.ts`, afin que la preuve de longue absence ne puisse révéler après coup une requête détachée utilisant un pool déjà fermé
- [X] T220 [US5] Préserver le brouillon visible d'une propriété structurée lorsqu'une projection de synchronisation rerend son parent avant le commit React, soumettre les valeurs DOM réelles et prouver les cinq profils cinq fois dans `apps/web/src/features/databases/property-editor.tsx`, `apps/web/tests/database-page-interaction.spec.tsx` et `tests/e2e/databases-views.spec.ts`
- [X] T221 [US5] Isoler chacun des dix rejeux de rollover de 512 updates dans sa propre frontière de test, sans réduire leur volume ni élargir le timeout générique, afin que l'instrumentation de couverture sur un petit runner mesure les mêmes invariants sans cumuler dix scénarios indépendants sous une seule horloge dans `packages/page-state/tests/checkpoints.property.spec.ts`
- [X] T222 [P] [US1] [US6] Verrouiller par tests composant et journey que le workspace ne rend que le contenu et des états compacts, que chaque surface déplacée reste accessible et que le retour restaure item, focus et scroll dans `apps/web/tests/workspace-content-boundary.spec.tsx` et `tests/e2e/workspace-settings-boundary.spec.ts`
- [X] T223 [US5] Découvrir par index les pages possédant des updates durables en attente, les drainer avec une concurrence bornée au boot, au retour online et après signal SSE sans rouvrir leur éditeur, agréger cet état dans le statut global et verrouiller le redémarrage sur une autre page dans `packages/client-core/src/local-store/schema.ts`, `packages/client-core/src/page-sync/encrypted-update-log.ts`, `apps/web/src/services/local-content.ts`, `apps/web/tests/synchronize-serialization.spec.ts`, `packages/client-core/tests/page-operation-schema.spec.ts`, `packages/client-core/tests/page-operation-encryption.spec.ts` et `tests/e2e/page-multi-device-convergence.spec.ts` per FR-073 et US5/AC8
- [X] T224 [US5] Reprendre également une branche legacy durable après fermeture de son éditeur, sans concurrencer une session encore montée, ordonner sa conversion derrière ses mutations workspace, retirer atomiquement toute autorité opérationnelle locale lors d'une conversion page vers dossier et répondre explicitement aux pulls tardifs côté serveur dans `packages/client-core/src/page-sync/encrypted-update-log.ts`, `packages/client-core/src/outbox/apply-local-mutation.ts`, `packages/client-core/src/outbox/apply-to-projection.ts`, `apps/web/src/services/local-content.ts`, `apps/api/src/page-state/page-operation-service.ts`, leurs tests ciblés et les journeys de recherche/conversion per FR-064, FR-073 et US5/AC8
- [X] T225 [US5] Activer une page historique directement depuis la tête canonique serveur avant le premier geste d'un éditeur connecté, drainer d'abord ses écritures workspace dépendantes, retenter une tête devenue stale de manière bornée, réserver la branche sémantique au vrai mode hors ligne et prouver qu'aucun `page.document.replace` ne part dans le parcours normal dans `apps/web/src/services/local-content.ts`, `apps/web/tests/operational-page-opening.spec.ts` et `tests/e2e/page-protocol-migration.spec.ts` per FR-061 et FR-064
- [X] T226 [US5] Faire attendre à chaque appelant d'une synchronisation rejointe la totalité des passes coalescées qu'il a demandées, afin qu'une barrière d'activation ne puisse pas observer prématurément une création encore en file, dans `apps/web/src/services/local-content.ts` et `apps/web/tests/synchronize-serialization.spec.ts` per FR-064 et FR-073
- [X] T227 [US5] Ne republier une page durable qu'après un changement matériel de lot, frontier, version vector, digest ou ambiguïtés afin qu'une confirmation vide ne crée pas une boucle projection/synchronisation, et verrouiller la seconde passe dans `packages/client-core/src/page-sync/page-reconciler.ts` et `packages/client-core/tests/page-reconciler.property.spec.ts` per FR-026, FR-054 et FR-059
- [X] T228 [US2] [US5] Séparer dans le témoin navigateur une adoption distante du premier geste local afin qu'une projection tardive ne masque pas la preuve de durabilité de la première saisie, dans `apps/web/src/features/editor/page-editor.tsx` et `tests/e2e/block-editor.spec.ts` per FR-026 et FR-064
- [X] T229 [US2] [US5] Centraliser côté domaine la distinction entre contenu éditorial et paragraphe vide structurel, l'appliquer identiquement à la projection optimiste et au serveur, puis couvrir la conversion d'une page activée mais intacte dans `packages/domain/src/content/conversion.ts`, `packages/client-core/src/outbox/apply-to-projection.ts`, `packages/database/src/repositories/content/conversion-repository.ts` et leurs tests per FR-064
- [X] T230 [US5] Fermer dans la transaction sérialisable la course entre activation opérationnelle et `page.document.replace`, conserver le problème 426 sur les routes directe et batch sans préempter le rejeu d'une mutation déjà acceptée, puis amorcer le benchmark de 500 blocs avant toute ouverture éditable dans `packages/database/src/mutations/execute-command.ts`, `apps/api/src/routes/page-documents.ts`, `apps/api/src/routes/mutation-batch.ts`, `packages/database/tests/page-operations.integration.spec.ts`, `apps/api/tests/page-operations.contract.spec.ts`, `apps/api/tests/page-documents.contract.spec.ts` et `tests/e2e/editor-performance.spec.ts` per FR-040, FR-061, FR-063 et SC-007
- [X] T231 [US2] Centraliser dans `tests/e2e/helpers.ts` l'attente d'activation et de synchronisation éditoriales avant toute interaction avec une surface fichier, puis l'appliquer à `tests/e2e/files.spec.ts`, `tests/e2e/accessibility.spec.ts`, `tests/e2e/file-preview.spec.ts` et `tests/e2e/narrow-viewport.spec.ts`, afin qu'un rafraîchissement concurrent ne remplace ni confirmation ni aperçu entre `pointerdown` et `pointerup` sur un moteur contraint, per FR-026 et SC-005
- [X] T232 [US5] Empêcher une activation ou une écriture éditoriale déjà préparée de recréer l'autorité opérationnelle après une conversion page vers dossier, en vérifiant le type courant dans la même transaction IndexedDB que chaque installation/commit et en protégeant le cache par compare-and-swap dans `packages/client-core/src/page-sync/migration.ts`, `packages/client-core/src/page-sync/local-page-state.ts`, `packages/client-core/src/local-store/local-repository.ts`, `apps/web/src/services/local-content.ts`, leurs tests déterministes et `tests/e2e/item-conversion.spec.ts` per FR-026, FR-064 et FR-073
- [X] T233 [US5] Rendre chaque revendication `sending` strictement propriétaire à son échange, libérer ce lot sur toute erreur avant commit, faire attendre sans transport un passage concurrent de la même page et verrouiller l'absence de requêtes vides ainsi que les références visuelles Linux dans `packages/client-core/src/page-sync/page-reconciler.ts`, `packages/client-core/tests/page-reconciler.property.spec.ts` et `tests/e2e/workspace-shell-visual.spec.ts-snapshots/` per FR-026, FR-054, FR-059 et FR-073

## Phase 12: Convergence — propriété inter-onglets et reprise à chaud

**Purpose**: Fermer l'écart entre la sérialisation en mémoire d'un onglet et la
coordination réellement nécessaire entre tous les contextes d'un même appareil,
sans modifier le protocole CRDT inter-appareils ni réintroduire un remplacement
de document complet.

- [X] T234 [P] [US5] Écrire avant correction les tests déterministes de deux handles IndexedDB et deux propriétaires de transport partageant le même nom de base : commits simultanés du même paragraphe, une seule revendication réseau par file, fermeture du propriétaire pendant `sending`, reprise par le successeur sans timeout métier et absence de reset d'un envoi vivant dans `packages/client-core/tests/cross-context-coordinator.spec.ts`, `packages/client-core/tests/page-operation-atomicity.spec.ts`, `packages/client-core/tests/page-reconciler.property.spec.ts` et `apps/web/tests/synchronize-serialization.spec.ts` per FR-052, FR-054, FR-060 et FR-073
- [X] T235 [P] [US5] Écrire un journey de deux onglets du même contexte qui éditent concurremment une page, voient les mises à jour durables sans rechargement, puis ferme l'onglet propriétaire au milieu d'un échange et vérifie la reprise automatique, la convergence du texte/IDs/statuts et l'absence de `page.document.replace` dans `tests/e2e/page-multi-tab-convergence.spec.ts` et `ci/test-impact.json` per US5/AC6, FR-053, FR-059, FR-060, FR-061 et SC-020
- [X] T236 [US5] Implémenter un coordinateur exclusif par origine fondé sur Web Locks, nommé uniquement par base/workspace/page, avec une file déterministe injectée hors navigateur, puis l'appliquer aux fenêtres d'écriture de projection/page et aux propriétés de transport workspace/page sans verrouiller deux fois la même ressource dans `packages/client-core/src/coordination/cross-context-coordinator.ts`, `packages/client-core/src/outbox/projection-write-coordinator.ts`, `packages/client-core/src/page-sync/page-write-coordinator.ts`, `packages/client-core/src/page-sync/page-reconciler.ts` et `apps/web/src/services/local-content.ts` per FR-052, FR-054, FR-060 et FR-073
- [X] T237 [US5] Déplacer la récupération des lignes `sending` sous le propriétaire exclusif qui vient d'être acquis, la borner à sa file/page, supprimer les resets de démarrage capables de préempter un autre onglet vivant et garantir qu'une libération de lock après fermeture reprend le même ID idempotent dans `packages/client-core/src/outbox/outbox.ts`, `packages/client-core/src/reconciliation/reconcile.ts`, `packages/client-core/src/page-sync/encrypted-update-log.ts`, `packages/client-core/src/page-sync/page-reconciler.ts` et `apps/web/src/services/local-content.ts` per FR-054, FR-059, FR-060 et FR-073
- [X] T238 [US5] Brancher le canal inter-onglets au service réel : publier seulement après commit local, vérifier/importer l'état partagé avant acquittement, adopter les deltas dans toute session ouverte, déclencher le drainage borné et fermer les canaux avec leur dernière session dans `packages/client-core/src/page-sync/tab-channel.ts`, `packages/client-core/src/page-sync/page-editing-session.ts`, `apps/web/src/services/local-content.ts`, `packages/client-core/tests/page-tab-channel.spec.ts` et `apps/web/tests/operational-page-opening.spec.ts` per FR-052, FR-059, FR-060 et US5/AC6
- [X] T239 [US5] Exécuter la nouvelle matrice onglets/crash/reconnexion sur les cinq profils, consigner les preuves et la décision Web Locks + BroadcastChannel dans `specs/017-v1-notion-like-workspace/validation.md`, `specs/017-v1-notion-like-workspace/plan.md` et `docs/architecture/synchronization.md`, puis lancer le gate exact de `docs/development.md` per SC-015, SC-020 et la Definition of Done
- [X] T240 [US5] Fermer la course où une adoption durable mise en file juste avant `beforeinput` pouvait déplacer la réplique sous les offsets d'une rafale locale : réévaluer la garde au démarrage de la demande, à son exécution et après reconstruction asynchrone, puis verrouiller la régression par test déterministe et répétitions du journey riche dans `packages/client-core/src/page-sync/page-editing-session.ts`, `packages/client-core/tests/page-editing-session.spec.ts` et `tests/e2e/rich-page.spec.ts` per FR-052, FR-059, FR-060 et SC-020

## Phase 13: Convergence — canevas de page focalisé et navigation cohérente

**Purpose**: Fermer les écarts observés sur l'interface réellement livrée sans
rouvrir le protocole de synchronisation : le titre appartient à la page, le
document occupe la surface de travail, les états techniques restent discrets et
les deux relations fichier d'une page demeurent distinctes conformément au
canvas produit.

- [X] T241 [P] [US1] [US3] Écrire avant correction les tests composant et journeys qui prouvent qu'une page présente son titre comme première grande ligne éditable, qu'un titre vidé devient « Sans titre » sans changer l'identité de l'item, que le renommage converge dans le fil d'Ariane et la barre latérale, et que le focus ainsi que le curseur restent stables pendant une adoption distante dans `apps/web/tests/page-title-editor.spec.tsx`, `tests/e2e/workspace-shell.spec.ts` et `tests/e2e/page-multi-device-convergence.spec.ts` per FR-008, FR-010, FR-026 et US1/AC5
- [X] T242 [P] [US1] [US6] Écrire avant correction les assertions de structure, accessibilité et références visuelles desktop/mobile, clair/sombre qui refusent un titre ou des réglages dupliqués dans le chrome, une carte autour du document, un panneau de pièces jointes sous le texte, un statut qui décale la page ou un libellé Réglages placé sous son icône dans `apps/web/tests/workspace-shell.spec.tsx`, `tests/e2e/workspace-settings-boundary.spec.ts`, `tests/e2e/workspace-shell-visual.spec.ts` et ses snapshots per FR-002, FR-007, FR-041, FR-043, FR-048, FR-072, SC-006, SC-010 et SC-014
- [X] T243 [P] [US3] Écrire avant correction les tests contrat, intégration et journey d'un fichier inséré dans un bloc : son upload finalisé crée idempotemment une pièce jointe de contenu placée sous la page source, jamais un élément de hiérarchie racine, conserve le même `fileItemId` après interruption/reprise et reste distinct d'un fichier autonome volontairement créé sous cette page dans `apps/api/tests/uploads.contract.spec.ts`, `packages/database/tests/uploads.integration.spec.ts`, `apps/web/tests/realtime-file-sync-status.spec.ts`, `tests/e2e/editor-offline-media.spec.ts` et `tests/e2e/files.spec.ts` per FR-023, FR-027, FR-028 et canvas 11.2/12
- [X] T244 [P] [US1] [US2] Écrire avant correction les tests de création de page et de la commande `/page` : une page créée depuis la navigation devient immédiatement active sur desktop/mobile ; `/page` crée une sous-page à la position hiérarchique courante en deux actions au plus, persiste dans le document un lien interne portant son identité canonique avant d'ouvrir cette nouvelle page, révèle la branche concernée et ne modifie pas le bloc courant si la création échoue dans `apps/web/tests/slash-menu.spec.ts`, `tests/e2e/workspace-shell.spec.ts` et `tests/e2e/page-links.spec.ts` per FR-010, FR-011, FR-022, FR-027, FR-028, FR-074 et SC-012
- [X] T245 [US1] Implémenter un titre de page éditable en place au-dessus du premier bloc dans `apps/web/src/features/workspace/page-title-editor.tsx`, `apps/web/src/features/hierarchy/hierarchy-explorer.tsx`, `apps/web/src/features/workspace/page-header.tsx` et `apps/web/src/features/workspace/workspace.css`, avec commit local sérialisé, libellé de repli « Sans titre », mise à jour optimiste de la navigation et aucune remonte du composant BlockNote per FR-008, FR-010, FR-026 et US1/AC5
- [X] T246 [US1] [US4] Remplacer le double affichage technique par un pied de page éditorial compact et stable : indicateur accessible sous un bouton d'information, détail au survol/focus/clic, actions de reprise toujours atteignables et annonces live non dupliquées ; retirer le statut et le bouton Réglages de l'en-tête tout en conservant diagnostics et réglages dans leurs surfaces dédiées dans `apps/web/src/components/sync-status.tsx`, `apps/web/src/features/editor/editor-sync-status.tsx`, `apps/web/src/features/editor/editor-view.tsx`, `apps/web/src/features/hierarchy/hierarchy-explorer.tsx` et les styles associés per FR-007, FR-026, FR-029, FR-041, FR-048 et FR-072
- [X] T247 [US1] [US6] Transformer la vue page en canevas continu occupant tout l'espace disponible hors barre latérale, réduire le chrome supérieur au fil d'Ariane compact, rendre les contrôles de création racine contextuels et aligner horizontalement icône et libellé Réglages en bas à gauche dans `apps/web/src/features/workspace/workspace-shell.tsx`, `apps/web/src/features/workspace/page-header.tsx`, `apps/web/src/features/hierarchy/hierarchy-explorer.tsx`, `apps/web/src/features/navigation/navigation.css` et `apps/web/src/features/workspace/workspace.css` per FR-001, FR-002, FR-005, FR-007, FR-043 et FR-072
...
- [X] T252 [P] [US3] Écrire avant correction les tests de staging chiffré, quota, crash à chaque frontière et reprise du même `fileItemId` dans `packages/client-core/tests/pending-file-transfer-store.spec.ts`, `apps/web/tests/realtime-file-sync-status.spec.ts` et `tests/e2e/editor-offline-media.spec.ts` per FR-023, FR-027, FR-071, FR-075, SC-020 et SC-022
- [X] T253 [US3] Ajouter à Dexie le manifeste et les morceaux chiffrés d'un transfert fichier, avec AAD liée à l'identité, migration, nettoyage des stagings incomplets et aucune donnée en clair dans `packages/client-core/src/local-store/schema.ts`, `packages/client-core/src/files/pending-file-transfer-store.ts` et les codecs de chiffrement locaux per FR-049, FR-067 et FR-075
- [X] T254 [US3] Faire précéder le commit du bloc par le staging durable, reconstruire et drainer toutes les files au lancement/online, reprendre l'offset serveur et supprimer les morceaux seulement après vérification dans `apps/web/src/features/editor/editor-files.ts`, `apps/web/src/features/editor/page-editor.tsx`, `apps/web/src/features/sync/use-page-reconciler.ts` et `apps/web/src/features/files/upload.ts` per FR-027, FR-071, FR-073 et FR-075
- [X] T255 [US3] Exécuter crash/rechargement hors ligne puis reconnexion sur les cinq profils, vérifier absence de doublon/orphelin/plaintext, documenter les preuves dans `specs/017-v1-notion-like-workspace/validation.md` et lancer le gate exact de `docs/development.md` per SC-013, SC-020, SC-021 et SC-022

## Phase 15: Convergence — navigation manipulable et édition sans chrome parasite

**Purpose**: Fermer les écarts UI observés après la stabilisation de la
synchronisation, sans modifier le protocole CRDT : présentation locale de la
sidebar, hiérarchie explicite, titre non intrusif, statut épinglé et cycle de
vie complet des liens.

- [X] T257 [P] [US1] Écrire avant correction les tests de normalisation et composants prouvant que `Favoris` et `Récents` peuvent être masqués puis repliés indépendamment, que les anciennes lignes IndexedDB reçoivent des valeurs sûres, que la préférence reste locale et que la hiérarchie porte le libellé `Notes` dans `packages/client-core/tests/navigation-state.spec.ts`, `apps/web/tests/sidebar.spec.tsx` et `apps/web/tests/settings-shell.spec.tsx` per FR-076, SC-026 et canvas 9/17
- [X] T258 [P] [US1] Écrire avant correction les tests unitaires, composants et journey d'une cible explicite `before|inside|after`, couvrant changement de parent, insertion entre deux voisins, refus cyclique, indicateur visuel préalable, indentation et état vide français rattaché au parent, ainsi que clic droit et chemin clavier sur une ligne dans `apps/web/tests/tree-drag-drop.spec.tsx`, `apps/web/tests/hierarchy-explorer.spec.tsx` et `tests/e2e/hierarchy.spec.ts` per FR-005, FR-077, SC-023 et US1/AC7-8
- [X] T259 [P] [US1] [US5] Écrire avant correction les tests par faux timers et géométrie qui gardent un titre vide pendant toute sa saisie focalisée, n'appliquent `Sans titre` qu'à la validation, maintiennent le témoin de synchronisation au bord inférieur visible sans déplacement du canevas et refusent son ouverture automatique dans `apps/web/tests/page-title-editor.spec.tsx`, `apps/web/tests/editor-sync-status.spec.tsx` et `tests/e2e/workspace-shell.spec.ts` per FR-078, FR-079, SC-024 et SC-027
- [X] T260 [P] [US2] Écrire avant correction les tests composant/adaptateur et journeys du cycle créer, reconnaître au pointeur, ouvrir, renommer, retargeter et retirer pour un lien Web et un `pageLink`, depuis toolbar, clic droit et clavier, en prouvant que le retrait conserve le texte et ne supprime jamais la page cible dans `apps/web/tests/editor-menus.spec.tsx`, `apps/web/tests/page-link-inline-content.spec.ts`, `tests/e2e/page-links.spec.ts` et `tests/e2e/block-editor.spec.ts` per FR-080, SC-025 et US2/AC6
- [X] T261 [US1] Étendre `WorkspacePresentationState` par quatre préférences normalisées, les exposer dans une section Réglages de navigation, rendre `Favoris` et `Récents` indépendamment visibles/repliables avec des contrôles compacts, puis renommer `Espace privé` en `Notes` dans `packages/client-core/src/navigation/presentation-state.ts`, `apps/web/src/features/navigation/sidebar.tsx`, `apps/web/src/features/settings/settings-shell.tsx` et leurs styles per FR-076 et SC-026
- [X] T262 [US1] Remplacer la cible unique de l'arbre par trois zones droppables encodant directement l'intention, traduire chaque zone en placement canonique après validation de cycle, afficher ligne ou parent actif, resserrer la marge gauche, rattacher visuellement l'état vide français et ouvrir les actions existantes de renommage/déplacement/suppression au clic droit comme au clavier dans `apps/web/src/features/navigation/tree-drag-drop.tsx`, `apps/web/src/features/hierarchy/hierarchy-explorer.tsx`, `apps/web/src/features/navigation/branch-state.tsx`, `apps/web/src/features/navigation/navigation-item-menu.tsx` et `apps/web/src/features/navigation/navigation.css` per FR-005, FR-077 et SC-023
- [X] T263 [US1] [US5] Séparer brouillon et valeur persistée du titre afin qu'un vide focalisé ne soit jamais remplacé par le debounce, commiter le repli seulement à la sortie/validation/navigation, puis sortir `EditorSyncStatus` du flux, l'épingler dans le viewport éditorial et garder ses détails fermés par défaut même en état d'attention dans `apps/web/src/features/workspace/page-title-editor.tsx`, `apps/web/src/features/editor/editor-sync-status.tsx`, `apps/web/src/features/editor/editor-view.tsx`, `apps/web/src/features/workspace/workspace.css` et `apps/web/src/global.css` per FR-078, FR-079, SC-024 et SC-027
- [X] T264 [US2] Conserver la toolbar BlockNote Community pour les liens Web et ajouter un contrôleur contextuel Ariakit pour `pageLink`, avec sélection canonique de cible, édition du libellé, retrait par conversion en texte, pointeur de lien, navigation interne et commandes partagées toolbar/clic droit/clavier sans supprimer la cible dans `apps/web/src/features/editor/editor-menus/formatting-toolbar.tsx`, `apps/web/src/features/editor/page-link-inline-content.ts`, `apps/web/src/features/editor/page-editor.tsx`, `apps/web/src/features/editor/editor-shortcuts.ts` et les styles associés per FR-080 et SC-025
- [X] T265 [US1] [US2] [US5] Exécuter les suites ciblées puis les journeys Chromium/Firefox/WebKit en parallèle, valider desktop/mobile et clair/sombre pour sidebar, arbre, titre, statut et liens, contrôler seulement l'ergonomie de base demandée — clavier, focus visible, souris et toucher —, consigner les preuves dans `specs/017-v1-notion-like-workspace/validation.md`, puis lancer le gate exact de `docs/development.md` sur le commit à pousser per SC-004, SC-005, SC-023 à SC-027 et la Definition of Done
- [X] T266 [US5] Empêcher un ancien chargement structuré de remplacer la sélection courante entre le commit React et le nettoyage de son effet, afin qu'une notification de synchronisation ne remonte pas le panneau d'entrée et n'efface pas un brouillon en cours ; verrouiller la course sur les cinq profils dans `apps/web/src/features/hierarchy/hierarchy-explorer.tsx` et `tests/e2e/databases-offline-sync.spec.ts` per FR-026, FR-052 et FR-059
- [X] T267 [P] Rendre le reset de sécurité E2E aussi résistant que le reset canonique aux victimes PostgreSQL `40P01`/`40001`, avec rollback et retries bornés, puis répéter vingt fois le parcours fautif sur chacun des cinq profils dans `tests/e2e/reset-installation.ts` et `tests/e2e/security-recovery.spec.ts` afin que le gate reste fiable sous activité serveur concurrente
- [X] T268 [US1] Différer l'activation clavier du menu d'une ligne d'arbre jusqu'à la fin de l'événement `Shift+F10`, afin que WebKit mobile ne restaure pas le focus sur le déclencheur en refermant immédiatement le menu, puis verrouiller clic droit et clavier vingt fois sur chacun des cinq profils dans `apps/web/src/features/hierarchy/hierarchy-explorer.tsx` et `tests/e2e/hierarchy.spec.ts` per FR-077, SC-023 et la portée clavier de base convenue

## Phase 16: Convergence — navigation lisible et cycle de lien unifié

**Purpose**: Fermer les écarts observés sur la version réellement utilisée,
sans rouvrir le protocole de synchronisation ni étendre la portée
d'accessibilité au-delà du clavier, du focus visible, du pointeur et du toucher.

- [X] T269 [P] [US1] Écrire les assertions composant et responsive puis placer un chevron iconographique immédiatement avant chacun des libellés Favoris et Récents, introduire un interrupteur booléen cohérent dans les réglages et prouver la persistance locale dans `apps/web/src/features/navigation/sidebar.tsx`, `apps/web/src/features/settings/workspace-navigation-settings.tsx`, `apps/web/src/ui/primitives/`, `apps/web/src/features/navigation/navigation.css`, `apps/web/src/features/settings/settings.css`, `apps/web/tests/sidebar.spec.ts` et `apps/web/tests/workspace-navigation-settings.spec.tsx` per FR-081 (partial)
- [X] T270 [P] [US1] Écrire le journey de retrait du dernier enfant puis normaliser les branches ouvertes afin qu'une page sans enfant redevienne une feuille sans état vide, remplacer les glyphes de branche par les chevrons communs et supprimer tout rail ou bord d'accent de la sélection au profit d'une surface neutre dans `apps/web/src/features/hierarchy/hierarchy-explorer.tsx`, `apps/web/src/features/navigation/branch-state.tsx`, `apps/web/src/features/navigation/navigation.css`, `apps/web/src/global.css`, `apps/web/tests/tree-drag-drop.spec.ts` et `tests/e2e/hierarchy.spec.ts` per FR-082 (contradicts)
...
- [X] T273 [US2] Reproduire la suppression du dernier caractère lié et la ligne vide sur les moteurs pris en charge, retirer tout mark stocké avant la saisie, le collage ou la composition suivante et rendre le caret explicitement visible dans les deux thèmes dans `apps/web/src/features/editor/page-editor.tsx`, `apps/web/src/features/editor/editor-links.ts`, `apps/web/src/global.css`, `apps/web/tests/editor-input.spec.ts` et `tests/e2e/block-editor.spec.ts` per FR-084 (partial)
...
## Phase 17: Convergence — identité d'item et deux familles de liens explicites

**Purpose**: Remplacer définitivement le parcours unifié de la phase 16 par
deux interactions compactes et prévisibles, faire de l'emoji une propriété
canonique de page/dossier et garantir que l'arbre, l'en-tête, les références,
l'offline et la synchronisation rendent la même identité. Cette phase ne lance
aucun chantier VoiceOver : elle valide le clavier, le focus visible, le
pointeur, le toucher et les moteurs pris en charge.

- [X] T275 [P] [US1] [US5] Écrire avant implémentation les tests domaine et contrat d'une icône nullable limitée à un grapheme emoji Unicode sur page/dossier, refusée sur fichier, transportée par création et mutation idempotente et compatible avec les réponses dépourvues du champ dans `packages/domain/tests/content-types.spec.ts`, `packages/contracts/tests/content-api.contract.spec.ts` et `apps/api/tests/content.contract.spec.ts` per FR-085 et SC-031
- [X] T276 [P] [US1] [US5] Écrire avant implémentation les tests de migration, repository, présentation protégée, projection Dexie chiffrée rétrocompatible, export, backup et restore qui conservent exactement l'emoji après offline/reconnexion et lisent une ancienne enveloppe de nom comme `{ name, icon: null }` dans `packages/database/tests/migrations.integration.spec.ts`, `packages/database/tests/content-repository.integration.spec.ts`, `packages/client-core/tests/local-record-codec.spec.ts`, `packages/client-core/tests/projection.spec.ts` et `packages/database/tests/reference-backups.integration.spec.ts` per FR-049, FR-067, FR-085 et SC-031
- [X] T277 [US1] [US5] Ajouter `icon` au modèle canonique, aux schémas HTTP/OpenAPI, à la commande idempotente, à la migration SQL, aux repositories, à la présentation serveur protégée, au change feed, à la projection locale chiffrée et aux exports/backups/restores dans `packages/domain/src/content/`, `packages/contracts/src/content-api.ts`, `apps/api/src/routes/items.ts`, `apps/api/src/protected-content/`, `packages/database/src/schema/`, `packages/database/src/repositories/`, `packages/database/migrations/`, `packages/client-core/src/local-store/` et `packages/database/src/backup/` per FR-085 et SC-031
- [X] T278 [P] [US1] Écrire avant implémentation les tests composant et journeys qui prouvent une icône d'item identique dans l'arbre, l'en-tête, la recherche et les références, un sélecteur emoji entièrement local utilisable au clavier, le fallback de type et le remplacement géométriquement stable de l'icône par le chevron au survol/focus d'une branche dans `apps/web/tests/item-icon.spec.tsx`, `apps/web/tests/emoji-picker.spec.tsx`, `apps/web/tests/hierarchy-explorer.spec.tsx`, `tests/e2e/hierarchy.spec.ts` et `tests/e2e/workspace-shell-visual.spec.ts` per FR-085, FR-086, FR-089, SC-031 et SC-033
- [X] T279 [US1] Créer la primitive `ItemIcon`, le sélecteur Emoji Mart embarqué et les actions ajouter/changer/retirer, puis les brancher au-dessus du titre, dans la ligne d'arbre, la recherche et les menus d'item ; faire partager à `ItemIcon` et au chevron une boîte fixe sans colonne de disclosure pour les feuilles dans `apps/web/src/ui/item-icon.tsx`, `apps/web/src/ui/emoji-picker.tsx`, `apps/web/src/features/workspace/`, `apps/web/src/features/hierarchy/hierarchy-explorer.tsx`, `apps/web/src/features/search/` et `apps/web/src/features/navigation/navigation.css` per FR-085, FR-086 et SC-033
- [X] T280 [P] [US2] Écrire avant implémentation les tests du sélecteur compact « Lien vers une page » : recherche nom/chemin, flèches, Entrée, Échap, clic, UUID canonique, titre et emoji résolus dynamiquement, pointeur, navigation interne, badge absent pour l'enfant direct mais présent ailleurs, retarget et retrait sans mutation de la cible dans `apps/web/tests/page-link-picker.spec.tsx`, `apps/web/tests/page-link-inline-content.spec.ts`, `apps/web/tests/editor-menus.spec.tsx` et `tests/e2e/page-links.spec.ts` per FR-080, FR-083, FR-087, FR-089, SC-031 et SC-032
- [X] T281 [US2] Remplacer le champ unifié par `PageLinkPicker`, rendre le node view `pageLink` non éditable et résolu depuis la projection courante, propager renommage/emoji sans réécrire le document, calculer le badge depuis la parenté et exposer ouvrir/retargeter/retirer via toolbar, `/page-link`, clic droit et clavier dans `apps/web/src/features/editor/editor-menus/page-link-picker.tsx`, `apps/web/src/features/editor/page-link-inline-content.ts`, `apps/web/src/features/editor/editor-menus/formatting-toolbar.tsx`, `apps/web/src/features/editor/editor-menus/slash-menu.tsx` et `apps/web/src/features/editor/page-editor.tsx` per FR-080, FR-083, FR-087, SC-031 et SC-032
- [X] T282 [P] [US2] Écrire avant implémentation les tests du dialogue compact « Lien Web » et du bookmark pleine ligne : URL obligatoire et autorisée, validation Entrée, fermeture Échap, rejet explicite, rendu stable avec URL/domaine sans métadonnée ni iframe, brouillon préservé pendant adoption distante et cycle ouvrir/modifier/retirer dans `apps/web/tests/web-bookmark-dialog.spec.tsx`, `apps/web/tests/embed-block.spec.tsx`, `apps/web/tests/slash-menu.spec.ts`, `apps/web/tests/editor-input.spec.ts` et `tests/e2e/page-links.spec.ts` per FR-026, FR-059, FR-083, FR-088, FR-089 et SC-034
- [X] T283 [US2] Créer `WebBookmarkDialog`, insérer un bloc `embed` `provider=bookmark` sur sa propre ligne, rendre une carte statique lisible hors ligne, séparer explicitement `/web-bookmark` de `/embed` et de `/page-link`, puis exposer ouvrir/modifier/retirer dans la toolbar, le menu de bloc et le clavier sans réutiliser le dialogue unifié dans `apps/web/src/features/editor/editor-menus/web-bookmark-dialog.tsx`, `apps/web/src/features/editor/custom-blocks/embed.tsx`, `apps/web/src/features/editor/editor-menus/formatting-toolbar.tsx`, `apps/web/src/features/editor/editor-menus/slash-menu.tsx` et `apps/web/src/features/editor/page-editor.tsx` per FR-083, FR-088 et SC-034
- [X] T284 [P] [US2] [US5] [US7] Verrouiller par tests moteur la ligne vide, le caret visible, la suppression du dernier caractère lié, la saisie/collage/composition suivante sans marque résiduelle et la conservation focus/sélection pendant une adoption distante dans `apps/web/tests/editor-input.spec.ts`, `apps/web/tests/operational-page-opening.spec.ts`, `tests/e2e/block-editor.spec.ts` et `tests/e2e/page-multi-device-convergence.spec.ts` per FR-026, FR-052, FR-084 et SC-030
- [X] T285 [US2] [US5] [US7] Corriger la boîte de ligne vide, `caret-color`, la frontière de marque ProseMirror et la restauration de sélection afin que Chromium, Firefox et WebKit affichent le caret et ne recréent jamais un lien supprimé dans `apps/web/src/features/editor/page-editor.tsx`, `apps/web/src/features/editor/editor-links.ts`, `apps/web/src/features/editor/page-editor.css` et `apps/web/src/global.css` per FR-026, FR-084 et SC-030
- [X] T286 [US1] [US2] [US7] Exécuter les journeys arbre/emoji/liens/caret sur les cinq profils avec thèmes clair/sombre et largeurs desktop/mobile, mettre à jour seulement les références visuelles intentionnelles et consigner les preuves fonctionnelles, géométriques, offline et clavier dans `tests/e2e/`, `tests/e2e/workspace-shell-visual.spec.ts-snapshots/` et `specs/017-v1-notion-like-workspace/validation.md` per SC-004, SC-005 et SC-031 à SC-034
- [X] T287 [US1] [US2] [US5] Mettre à jour l'état d'avancement et les contrats affectés, exécuter les suites ciblées en parallèle puis le gate exact `bun run checks:local` sur le commit à pousser, corriger toute régression, pousser la branche et ouvrir la PR dans `specs/017-v1-notion-like-workspace/tasks.md`, `specs/017-v1-notion-like-workspace/validation.md`, `docs/product/product-canvas.md`, `docs/development.md` et `.github/workflows/ci.yml` per Definition of Done

## Phase 18: Convergence — ligne d'arbre et pièces jointes compactes

**Purpose**: Implémenter la navigation validée sans rouvrir les chantiers de
synchronisation, de liens ou d'accessibilité avancée : ligne entière
déplaçable, création enfant intégrée, pièces jointes raccordées et identité de
dossier modifiable.

- [X] T288 [P] [US1] Écrire avant implémentation les tests source/composant qui interdisent la poignée de sidebar, exigent la ligne comme prise DnD, verrouillent l'ordre pièces jointes/`+`/`…`, la surface de création sans changement de géométrie et la présentation compacte vide/remplie dans `apps/web/tests/hierarchy-explorer.spec.tsx`, `apps/web/tests/tree-drag-drop.spec.ts`, `apps/web/tests/navigation-inline-create.spec.tsx` et `apps/web/tests/attachment-panel.spec.tsx` per FR-090 à FR-093
- [X] T289 [P] [US1] Écrire avant implémentation les journeys pointeur et clavier qui déplacent une ligne, créent une page et un dossier enfant, vérifient l'ouverture immédiate de la page, la géométrie stable des pièces jointes et l'édition titre/emoji d'un dossier dans `tests/e2e/hierarchy.spec.ts`, `tests/e2e/keyboard-navigation.spec.ts`, `tests/e2e/files.spec.ts` et `tests/e2e/workspace-shell.spec.ts` per FR-090 à FR-094
- [X] T290 [US1] Composer le draggable et la cible intérieure sur la ligne, supprimer `TreeDragHandle` de la sidebar, ignorer les contrôles interactifs et conserver les trois intentions de dépôt ainsi que les actions clavier existantes dans `apps/web/src/features/navigation/tree-drag-drop.tsx`, `apps/web/src/features/hierarchy/hierarchy-explorer.tsx` et `apps/web/src/features/navigation/navigation.css` per FR-090
- [X] T291 [US1] Construire les actions contextuelles ordonnées et la surface de création en ligne avec icônes page-plus/dossier-plus, rotation `+` vers croix, fermeture symétrique et menu complémentaire à une seule ligne par action dans `apps/web/src/features/navigation/navigation-inline-create.tsx`, `apps/web/src/features/navigation/navigation-item-menu.tsx`, `apps/web/src/ui/icons.tsx`, `apps/web/src/features/hierarchy/hierarchy-explorer.tsx` et `apps/web/src/features/navigation/navigation.css` per FR-091 et FR-093
- [X] T292 [US1] Ajouter la région repliable paresseuse, raccorder le panneau de pièces jointes à la ligne sélectionnée, rendre l'en-tête/compteur/état vide et les lignes nom-taille compacts sans retirer l'import ni les actions de fichier, puis animer descendants et panneaux sans espace résiduel dans `apps/web/src/features/navigation/collapsible-region.tsx`, `apps/web/src/features/attachments/attachment-panel.tsx`, `apps/web/src/features/hierarchy/hierarchy-explorer.tsx` et `apps/web/src/features/navigation/navigation.css` per FR-092 et FR-093
- [X] T293 [US1] Généraliser l'éditeur d'identité de page à page/dossier et rendre le titre ainsi que l'emoji du dossier ouvert modifiables par les mutations existantes dans `apps/web/src/features/workspace/page-title-editor.tsx`, `apps/web/src/features/hierarchy/hierarchy-explorer.tsx`, `apps/web/src/features/workspace/workspace.css` et `apps/web/tests/page-title-editor.spec.tsx` per FR-085 et FR-094
- [X] T294 [US1] Exécuter en parallèle les suites ciblées arbre, fichiers, shell, liens et curseur, puis le gate exact `bun run checks:local` sur le commit à pousser ; consigner preuves, impact CI et absence de régression de synchronisation/liens dans `specs/017-v1-notion-like-workspace/validation.md` et `specs/017-v1-notion-like-workspace/tasks.md` per Definition of Done
- [X] T295 [US1] Remplacer le disclosure natif intermittent des actions de pièce jointe par le popover partagé, garder son contenu dans le tiroir mobile, vérifier clic, `Échap`, retour de focus et `Entrée`, puis répéter les parcours fichiers sur les cinq profils dans `apps/web/src/features/attachments/attachment-panel.tsx`, `apps/web/src/features/navigation/navigation.css`, `apps/web/tests/attachment-panel.spec.tsx`, `tests/e2e/helpers.ts` et `tests/e2e/files.spec.ts` per FR-093 et la portée clavier de base convenue
- [X] T296 [US2] Tolérer les événements `beforeinput` WebKit sans `inputType`, conserver la sélection de lien au-dessus des remontages de la toolbar et verrouiller l'ouverture du sélecteur interne sans erreur JavaScript sur WebKit mobile dans `apps/web/src/features/editor/page-editor.tsx`, `apps/web/src/features/editor/editor-menus/formatting-toolbar.tsx` et `tests/e2e/block-editor.spec.ts` per FR-084, FR-087 et SC-032
- [X] T297 [US1] Garder l'instance Emoji Mart montée lorsque le parent renouvelle son callback, acheminer la sélection vers le callback courant sans remplacer le nœud focalisé et répéter le journey d'identité emoji sur Chromium mobile dans `apps/web/src/ui/emoji-picker.tsx`, `apps/web/tests/emoji-picker.spec.tsx` et `tests/e2e/hierarchy.spec.ts` per FR-085, FR-086 et SC-031

## Phase 19: Convergence — conformité à la maquette de navigation versionnée

**Purpose**: Reproduire sans réduction la référence validée
`assets/sidebar-attachments-v3.html`, y compris sa géométrie et ses états
intermédiaires, puis remplacer les contrôles seulement présents par des
interactions réellement conformes.

- [X] T298 [US1] Écrire avant correction les tests composant, CSS et Playwright qui utilisent `specs/017-v1-notion-like-workspace/assets/sidebar-attachments-v3.html` comme référence traçable, mesurent les rectangles avant/pendant/après, figent le nœud unique du chevron et sa rotation intermédiaire, la largeur progressive de la sidebar, la surface de création contenue et la hauteur progressive des descendants/pièces jointes sur desktop et mobile dans `apps/web/tests/hierarchy-explorer.spec.tsx`, `apps/web/tests/navigation-inline-create.spec.tsx`, `apps/web/tests/workspace-shell.spec.tsx`, `tests/e2e/hierarchy.spec.ts`, `tests/e2e/files.spec.ts` et `tests/e2e/workspace-shell.spec.ts` per SC-035 à SC-038 et Constitution III
- [X] T299 [US1] Ajouter dans l’en-tête ouvert la commande `PanelLeftClose`, conserver le panneau desktop monté pendant largeur/translation/opacité, rendre ses contrôles inertes une fois masqués, exposer `PanelLeftOpen` dans le contenu, transférer le focus après le mouvement et préserver `sidebarOpen` au rechargement dans `apps/web/src/features/navigation/responsive-sidebar.tsx`, `apps/web/src/features/navigation/navigation.css`, `apps/web/src/features/workspace/workspace-shell.tsx`, `apps/web/src/features/workspace/workspace.css` et `apps/web/src/ui/icons.tsx` per FR-095 et US1/AC13
- [X] T300 [P] [US1] Conserver un seul `ChevronRight` superposé à l’emoji, animer sa rotation 0→90 degrés et son retour, puis appliquer les surfaces de survol/focus et l’état ouvert de la référence sans déplacer le titre dans `apps/web/src/ui/item-icon.tsx` et `apps/web/src/features/navigation/navigation.css` per FR-086 et SC-036
- [X] T301 [P] [US1] Corriger la primitive repliable afin que descendants et pièces jointes restent montés pendant 210 ms, traversent une géométrie et une opacité intermédiaires dans les deux sens, ne chargent paresseusement qu’à la première ouverture et ne laissent ensuite aucun espace ni contrôle actif, avec réduction de mouvement respectée dans `apps/web/src/features/navigation/collapsible-region.tsx` et `apps/web/src/features/navigation/navigation.css` per FR-093, SC-036 et SC-038
- [X] T302 [US1] Recomposer la création enfant comme la surface intégrée de la référence corrigée — cluster 28 px, enveloppe desktop visible de 88 × 30 px avec respiration uniforme de 1 px, commandes page/dossier/croix réellement contenues dans cette même enveloppe, même `+` tourné en croix, aucune ombre extérieure, fermeture inverse conservée et trombone temporairement masqué — sans dépasser les limites supérieure, inférieure ou droite ni changer le rectangle de ligne dans `apps/web/src/features/navigation/navigation-inline-create.tsx`, `apps/web/src/features/hierarchy/hierarchy-explorer.tsx` et `apps/web/src/features/navigation/navigation.css` per FR-091 et SC-037
- [X] T303 [US1] Remplacer l’icône fichier par `Paperclip`, raccorder le panneau compact à la largeur de la ligne sélectionnée avec les coins et états visuels de la référence, préserver son cadre pendant le chargement asynchrone, puis valider visuellement les quatre états de la maquette, consigner les captures et exécuter le gate exact avant push dans `apps/web/src/ui/icons.tsx`, `apps/web/src/features/hierarchy/hierarchy-explorer.tsx`, `apps/web/src/features/attachments/attachment-panel.tsx`, `apps/web/src/features/navigation/navigation.css` et `specs/017-v1-notion-like-workspace/validation.md` per FR-092 et SC-038 (gate complet passé sur `d70ec9ca`)

## Phase 20: Stabilisation CI — transitions réelles et projection distante sans résurrection

**Purpose**: Fermer les deux courses révélées uniquement sous la charge de la
première CI de conformité, sans relâcher les assertions visuelles ni masquer un
défaut de synchronisation par une attente de test.

- [X] T304 [US1] Démonter une région repliable paresseuse sur son véritable événement `transitionend`, conserver un fallback borné uniquement pour les moteurs qui omettent cet événement et prouver par faux timers que WebKit peut observer toute la fermeture même lorsque la transition est mise en pause dans `apps/web/src/features/navigation/collapsible-region.tsx` et `apps/web/tests/collapsible-region.spec.tsx` per FR-093, SC-036 et SC-038
- [X] T305 [US2] Appliquer immédiatement un front distant adopté lorsque ni saisie ni commit local ne possède les offsets visibles, différer uniquement pendant une vraie activité locale et verrouiller la suppression d'un lien suivie d'une saisie sur la même ligne par tests unitaires et vingt répétitions Firefox Linux sans retry dans `apps/web/src/features/editor/editor-remote-apply.ts`, `apps/web/src/features/editor/page-editor.tsx`, `apps/web/tests/editor-remote-apply.spec.ts` et `tests/e2e/page-links.spec.ts` per FR-026, FR-052, FR-084 et SC-025
- [X] T306 [US1] [US2] Exécuter le gate exact `bun run checks:local` sur le commit corrigé, pousser, obtenir toute la matrice CI verte, fusionner puis vérifier la CI de `main` avant de mettre à jour l'instance HTTPS locale dans `specs/017-v1-notion-like-workspace/validation.md` per Definition of Done (PR #159, CI complète et cinq profils Playwright verts ; merge `2a369bde`, CI `main` verte et images exactes déployées sur l'instance HTTPS locale sans réinitialiser les volumes)

## Phase 21: Convergence — viewport fixe, création focalisée et reprise éditoriale

**Purpose**: Fermer les régressions observées après la conformité de la
maquette : scroll du shell, placement de la bascule, création sans
préformulaire, focus du titre, chargement éditorial, suppression définitive des
liens et état visuel du chevron.

- [X] T307 [P] [US1] Écrire les tests composant, CSS et Playwright qui prouvent que le document racine ne défile pas, que le contenu et l'arbre possèdent leurs scrollers indépendants, que le chrome, la sidebar et son pied restent fixes et que la commande de réouverture est alignée dans la ligne supérieure sans recouvrir le contenu dans `apps/web/tests/workspace-shell.spec.tsx`, `apps/web/tests/sidebar.spec.tsx`, `tests/e2e/workspace-shell.spec.ts` et `tests/e2e/workspace-shell-visual.spec.ts` per FR-095, FR-096, SC-035 et SC-039
- [X] T308 [P] [US1] Écrire les tests composant et journeys qui ouvrent `+ Nouveau` sans champ de nom, observent le même plus tourner en croix, créent page et dossier depuis la racine et un parent puis vérifient navigation, titre vide focalisé et fallback au blur dans `apps/web/tests/navigation-inline-create.spec.tsx`, `apps/web/tests/page-title-editor.spec.tsx`, `tests/e2e/hierarchy.spec.ts` et `tests/e2e/workspace-shell.spec.ts` per FR-097, FR-098, SC-012 et SC-040
- [X] T309 [P] [US2] Reproduire le geste exact de suppression au clavier d'une ligne contenant un `pageLink`, saisir du texte normal, attendre le commit, une projection distante et la synchronisation, puis prouver que la référence ne réapparaît jamais sur Chromium, Firefox et WebKit dans `packages/page-state/tests/rich-text.property.spec.ts` et `tests/e2e/page-links.spec.ts` per FR-084 et SC-030
- [X] T310 [US1] Borner le workspace au viewport, déplacer les overflow vers le contenu et l'arbre, garder le footer de navigation fixe et ancrer l'unique commande desktop de réouverture dans le conteneur supérieur avec transfert de focus symétrique dans `apps/web/src/features/workspace/workspace-shell.tsx`, `apps/web/src/features/workspace/workspace.css`, `apps/web/src/features/navigation/responsive-sidebar.tsx`, `apps/web/src/features/navigation/sidebar.tsx` et `apps/web/src/features/navigation/navigation.css` per FR-095, FR-096 et SC-039
- [X] T311 [US1] Étendre la machine d'état de création compacte à la racine, retirer le préformulaire de nom, conserver la création de base comme action secondaire, factoriser la sélection du nouvel item et transmettre une intention de focus à usage unique au titre pour racine, enfant et `/page` après sa frontière durable dans `apps/web/src/features/navigation/navigation-inline-create.tsx`, `apps/web/src/features/hierarchy/hierarchy-explorer.tsx`, `apps/web/src/features/workspace/page-title-editor.tsx` et `tests/e2e/helpers.ts` per FR-097, FR-098 et SC-040
- [X] T312 [US2] Remplacer l'état orange de préparation par un squelette éditorial neutre et rendre la marque CRDT atomique `pageLink` non expansible afin qu'une référence supprimée ne puisse être héritée par la nouvelle saisie dans `apps/web/src/features/editor/editor-view.tsx`, `apps/web/src/features/workspace/page-content-skeleton.tsx`, `packages/page-state/src/rich-text.ts` et les styles associés per FR-084, FR-099, SC-030 et SC-041
- [X] T313 [US1] Renforcer la surface du chevron au survol, au focus et à l'état ouvert sans déplacer l'identité, puis vérifier clair/sombre, souris, clavier et réduction de mouvement dans `apps/web/src/features/navigation/navigation.css`, `apps/web/tests/hierarchy-explorer.spec.tsx` et `tests/e2e/hierarchy.spec.ts` per FR-086, SC-033 et SC-036
- [ ] T314 [US1] [US2] Exécuter les suites ciblées en parallèle puis le gate exact `bun run checks:local`, consigner les captures et preuves dans `specs/017-v1-notion-like-workspace/validation.md`, pousser, obtenir la CI complète verte, fusionner, vérifier `main` puis mettre à jour l'instance HTTPS locale sans réinitialiser ses volumes per SC-005, SC-010, SC-030, SC-039 à SC-041 et Definition of Done (gate local complet vert ; CI, fusion, `main` et déploiement en attente)
- [X] T315 [US5] Reproduire les refus Firefox/WebKit de restauration et de conversion legacy lorsqu'une révision de métadonnées descend de la frontière éditoriale, centraliser le verrou de tête canonique, accepter uniquement l'écart d'ancêtre prouvé, rattacher toute nouvelle frontière sémantique à la tête canonique et verrouiller la régression par intégrations PostgreSQL ainsi que par les cinq profils Playwright dans `packages/database/src/repositories/item-reader.ts`, `apps/api/src/page-state/legacy-branch-service.ts`, `apps/api/src/page-state/page-history-service.ts`, `apps/api/src/page-state/page-operation-service.ts`, `apps/api/tests/page-operation-migration.integration.spec.ts`, `apps/api/tests/page-history-consolidation.integration.spec.ts` et `tests/e2e/` per FR-064, FR-069, SC-005 et SC-009

## Phase 22: Convergence — prise de ligne, fantôme et curseur de déplacement

**Purpose**: Rendre visible que toute la ligne d'arbre se saisit, montrer où
elle va, et garder le curseur de prise en cours pendant tout le geste.
