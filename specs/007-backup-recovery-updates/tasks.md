...
- [X] T001 Add `MYOWNNOTION_BACKUP_DESTINATION`, `MYOWNNOTION_BACKUP_ROOT`, `MYOWNNOTION_BACKUP_HOUR` and `MYOWNNOTION_BACKUP_RETENTION_DAYS` to `.env.example`, each with the default and the failure it prevents
- [X] T002 [P] Record the backup and restore commands in `docs/development.md` beside the existing administrative ones
...
- [X] T008 `apps/api/src/backup/destinations/destination.ts` — the four-operation boundary, including destination read-back, and `filesystem.ts` implementing it
...
- [X] T017 [P] [US2] `apps/api/src/admin/commands/backup-commands.ts` — `backup run` and `backup verify` with the exit codes of [contracts/admin-commands.md](./contracts/admin-commands.md)
- [X] T018 [P] [US2] `apps/web/src/features/backup/backup-panel.tsx` — when the last verified backup succeeded, and the 26-hour warning stated plainly
- [X] T019 [US2] Retention pass that deletes only after confirming a more recent verified backup remains
- [X] T020 [P] [US2] Playwright journey in `tests/e2e/backup.spec.ts` — the panel shows a verified backup, and the warning appears when the last one is stale
...
- [X] T026 [US1] Record a `restoration_attempt` when it starts, and finish it — an unfinished row is how an interruption is recognised
- [X] T027 [US1] Refuse to report health at startup while an unfinished restoration exists, and say how to resume or roll back (FR-017)
- [X] T028 [P] [US1] `apps/api/src/admin/commands/restore-apply.ts` — `--dry-run`, `--yes`, and no assumed consent when there is no terminal
...
- [X] T032 [US3] `apps/api/src/admin/commands/restore-test.ts` — the safe rehearsal, needing no confirmation
- [X] T033 [P] [US3] `apps/web/src/features/backup/restore-rehearsal.tsx` — the date, the result, the invitation after a month, and no secret
- [X] T034 [P] [US3] Playwright journey in `tests/e2e/backup.spec.ts` — a rehearsal is offered, run, and its result shown
...
- [X] T035 [P] [US4] Integration test in `packages/database/tests/update-guard.integration.spec.ts` — a failed backup verification stops the migration, and the schema is unchanged
...
- [X] T041 [P] [US4] `apps/api/src/admin/commands/version-inspect.ts`
...
- [X] T042 `apps/api/src/backup/destinations/google-drive.ts` behind the same four operations, with credentials mounted rather than stored
- [X] T043 [P] Contract test that the Drive destination satisfies the same boundary suite as the filesystem one, against a recorded interaction

---

## Phase 8: Polish

- [X] T044 [P] `docs/architecture/backup.md` — why the archive is the export, why a rehearsal writes, and where the update guard lives
- [X] T045 [P] Accessibility pass over the backup panel and the rehearsal invitation; add them to `tests/e2e/accessibility.spec.ts`
- [X] T046 [P] Narrow-viewport pass at 320 px, asserted in `tests/e2e/narrow-viewport.spec.ts`
- [X] T047 CI job restoring a reference backup for every supported migration path (SC-009)
- [X] T048 Measure a backup and a restore on a workspace with a thousand items and record the figures in `validation.md`
- [X] T049 Write `specs/007-backup-recovery-updates/validation.md` with evidence per requirement, marking anything unfinished as unfinished rather than ticking it

---

## Phase 9: Convergence — interrupted destination transfers

- [X] T050 [US2] Add regression coverage in `apps/api/tests/backup-archive.contract.spec.ts` for a destination that fails halfway: the locally verified backup and failed after-transfer verification remain recorded, while a completed but corrupt transfer remains addressable for re-verification
- [X] T051 [US2] Preserve the distinction between transferred and verified in `apps/api/src/backup/backup-service.ts` and `apps/api/src/admin/commands/backup-commands.ts`, returning a safe integrity failure instead of losing the run when the destination becomes unavailable (US2 acceptance scenario 2, FR-008, partial)
- [X] T052 [US2] Expose the latest after-creation and after-transfer outcomes safely through `packages/database/src/repositories/backup-repository.ts`, `apps/api/src/routes/backups.ts`, and `apps/web/src/features/backup/backup-panel.tsx`, with API and component tests that warn specifically about a failed transfer (FR-013 and the unreachable-destination edge case, partial)
- [X] T053 Update `specs/007-backup-recovery-updates/validation.md` with the interrupted-transfer evidence and the final executed checks
