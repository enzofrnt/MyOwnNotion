...
Feature 002 owns the baseline secure Compose, environment contract, external
reverse-proxy boundary, CI quality gates, GHCR publication, and immutable
release foundation required by FR-030 through FR-035. Feature 007 owns backup
scheduling, encrypted transfer, restore orchestration, update safeguards, and
their final V1 operational validation. Feature 008 owns private workspace
search and its protected derived indexes. Neither feature duplicates this
baseline security foundation.

Feature 021, [Readable Server Logs](../021-readable-server-logs/spec.md),
refines the operator experience of this baseline after real Compose use showed
that a machine-parseable non-interactive stream is not by itself a readable
local view. Feature 002 remains authoritative for redaction, audit separation,
standard container streams, and required event metadata. Feature 021 owns the
independent format/color/verbosity controls, routine-noise policy, safe
actionable diagnostics, and the final V1 usability evidence.

## Clarifications

### Session 2026-08-17

- Q: How should colors be reconciled with structured container logs? → A: Enable colors automatically for a compatible interactive terminal, disable them automatically otherwise, and provide an explicit configuration override. Feature 021 later separates color from presentation format so a non-interactive local stream can still receive a human-readable monochrome view without weakening the structured collector path.

### Session 2026-09-22

- Q: Must first-run setup force recovery-kit download and offline confirmation before the installation is ready? → A: No. Setup is exactly: open → create passkey → create password. Recovery-kit download and offline confirmation live in settings after the installation is usable. Incomplete setup never commits ownership; another browser may abandon the incomplete attempt and finish correctly.
- Q: May a second browser claim bootstrap while another attempt is incomplete? → A: Yes. Starting a new attempt abandons any incomplete open attempt. Only the first fully completed attempt commits the sole owner; afterwards bootstrap is closed.
- Product canvas alignment: sections 8 and 28.4 updated in the same change so feature 002 does not contradict product direction.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Establish the Sole Owner Securely (Priority: P1)

As the person operating a new installation, I can establish its one owner with a passkey and a password so that no unclaimed or second identity can take ownership, without being forced through recovery-kit download during setup.

**Why this priority**: Every later content and security operation depends on an unambiguous owner. Recovery readiness remains available afterwards in settings.

**Independent Test**: Start with an empty installation and valid external deployment key material, verify the uninitialized counts, complete first-run setup (passkey then password), verify counts become `1/1` and the installation is `ready` without any recovery-kit download, verify an incomplete attempt in another browser does not block a later complete attempt, and verify recovery-kit preparation remains available from settings after sign-in.

**Acceptance Scenarios**:

1. **Given** a fresh installation with valid externally supplied key material and no committed owner or workspace row, **when** the first-run operator verifies the bootstrap passkey, **then** verification remains inside the same bootstrap attempt, the installation remains pre-confirmation with `ownerCount=0` and `workspaceCount=0`, and no owner or workspace row is committed.
2. **Given** a bootstrap attempt with a verified passkey, **when** the operator sets an acceptable password and the attempt confirms, **then** one atomic transition commits the sole owner with both the passkey and the password alternative, binds the existing feature-001 canonical workspace identity, marks the installation `ready`, and changes the reported counts together from `0/0` to `1/1`, without requiring recovery-kit download or offline-storage confirmation.
3. **Given** an uninitialized installation or any pre-confirmation bootstrap state, **when** status is reported or another browser starts setup, **then** the status reports exactly `ownerCount=0` and `workspaceCount=0`, the incomplete prior attempt is abandoned rather than locking the installation, and no second owner is created.
4. **Given** recovery readiness after ownership is committed, **when** the owner opens settings, **then** they can prepare, download, and confirm a recovery kit there; setup itself MUST NOT require those steps.
...
10. **Given** an authenticated replacement preparation that has not yet been downloaded, **when** the owner requests status or the one-time download while preparation is running or changing, **then** the result represents one coherent preparation and never announces a downloadable opportunity before its artifact can be served; expiry, replacement, consumption, service unavailability, or restart makes the unclaimed opportunity unavailable, a restart safely requires a new preparation, and the official V1 deployment keeps preparation and download under the same transient-custody authority.
...
- Bootstrap is interrupted after passkey verification or after password recording but before confirmation; every pre-confirmation state remains `ownerCount=0` and `workspaceCount=0` with no committed owner/workspace rows.
- A second browser starts setup while another incomplete attempt is open; the incomplete attempt is abandoned, the new attempt proceeds, and only a completed confirmation commits ownership.
- A provisional recovery kit prepared from settings becomes downloadable, becomes download-consumed only after the one-time download is consumed, and becomes active only after explicit offline confirmation; a lost or expired unconfirmed opportunity is regenerated, rejects the prior delivery, and does not expire an active confirmed kit.
...
- **FR-001**: The installation MUST have exactly one owner identity and exactly one owner workspace; it MUST provide no second account, team member, private guest, role, or multi-owner path. All uninitialized and pre-confirmation bootstrap states—including passkey-verified, password-set, rejected, abandoned, and any superseded incomplete attempt—MUST have no committed owner or workspace rows and MUST report exactly `ownerCount=0` and `workspaceCount=0`. Only one atomic bootstrap transition, after a verified passkey and an acceptable password have both been recorded against the same attempt, MAY commit the sole owner with both credentials, bind the existing feature-001 canonical workspace identity, mark the installation `ready`, and change the counts to exactly `ownerCount=1` and `workspaceCount=1`; every initialized state—`recovery-required`, `ready`, `migration-in-progress`, and `degraded`—MUST report exactly `1` and `1`. That atomic transition MUST NOT require recovery-kit download or offline-storage confirmation.
- **FR-002**: The installation MUST provide protected first-run bootstrap that can be completed only once after ownership commits, whose operator-facing steps are exactly: open setup, create the owner passkey, create the password alternative. Incomplete attempts MUST NOT commit ownership and MUST NOT permanently lock the installation to one browser: starting a new attempt while ownership is still `0/0` MUST abandon any incomplete open attempt so the later complete attempt can finish. Bootstrap MUST keep passkey verification and password recording inside one attempt without committing owner or workspace rows, remain `0/0` until that atomic confirmation, and close permanently once an owner exists. Any bootstrap capability issued to the browser MUST be returned in the bootstrap response body or headers, MUST never be placed in a URL, logs, or persistent plaintext, and MUST be treated as a browser-held value rather than a request-only value.
...
- **FR-004**: First-run bootstrap MUST require setting the password alternative before ownership commits. After ownership exists, the owner-facing credential-management experience MUST allow the owner to change the password alternative and authenticate with it as an alternative to passkey-only sign-in without weakening the passkey-only path. Changing the password alternative after bootstrap MUST require recent authentication.
- **FR-005**: The owner-facing credential-management experience MUST allow the owner to list enrolled passkeys, enroll or add a passkey, and remove a selected passkey. Listing, enrolling or adding, and removing passkeys MUST be attributable to the owner session, and every post-bootstrap credential change—passkey enrollment/addition, selected-passkey removal, or password-alternative change—MUST require recent authentication; these owner-facing operations MUST NOT introduce a remote administrator API.
...
- **FR-015**: The installation MUST allow the owner to generate an encrypted recovery kit with two separate state axes: authorization state `provisional`, `active`, `superseded`, `revoked`, or `rejected`; and delivery/confirmation state `prepared`, `downloadable`, `download-consumed`, `confirmed`, or `expired`. Recovery-kit preparation, one-time download, and offline-storage confirmation MUST be available from authenticated owner settings after bootstrap, MUST require recent authentication for replacement after an active kit exists, and MUST NOT be required steps of first-run bootstrap. An installation MAY be `ready` with `1/1` before any recovery kit is prepared; until a kit reaches authorization `active` and delivery/confirmation `confirmed`, recovery readiness MUST be shown as incomplete in settings. The kit MUST identify its format and source installation lineage and MUST not be placed automatically beside the encrypted workspace.
- **FR-016**: Safe regeneration of an unconfirmed provisional recovery delivery MUST issue a new provisional kit and one new download opportunity, MUST leave the prior kit material rejected and its prior download opportunity expired, and MUST never revive an expired download opportunity. After an active confirmed kit exists, recovery-kit replacement MUST require recent authentication; the active confirmed kit MUST remain valid and unchanged until the replacement is downloaded, consumed, and explicitly confirmed offline, after which the replacement MUST become active and confirmed, the prior kit MUST become superseded, and the recovery epoch MUST advance atomically. Expiry MUST affect only an unconfirmed download opportunity and MUST never expire an active confirmed kit.
...
- **FR-020**: The supported V1 hosting-administrator command surface MUST be exposed only through protected local CLI operations and MUST include password reset, session revocation, data-integrity verification, key-availability verification without displaying keys, both rotation triggers and status inspection, migration inspection, compatible recovery, a protected local CLI compatibility-inspection operation, and redacted diagnostics; there MUST be no remote administrator API, remote administrator session, remote administrator bearer channel, or placeholder administrator authentication scheme. Owner-facing authenticated API operations MUST continue to use the owner session and recent authentication where required. Backup and restore operations remain owned by feature 007.
...
- **Password credential**: The password alternative required during first-run bootstrap and changeable afterwards with recent authentication.
...
- **SC-002**: Across at least 20 clean-install trials performed by at least 5 representative self-host operators, at least 19 trials complete first-run passkey creation, password creation, and the resulting atomic owner/workspace commit within 5 minutes after all prerequisites are available, without requiring recovery-kit download during setup; every observed pre-confirmation state remains `0/0`.
...
- Backup scheduling, remote backup transfer, retention, general restore
  orchestration, and update rollback remain owned by feature 007. Protected
  private search indexes remain owned by feature 008. This feature defines only
  the recovery-kit, encryption, and key-management contracts those features
  must honor.
