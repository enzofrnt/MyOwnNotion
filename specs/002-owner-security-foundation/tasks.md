...
- [X] T095 [US6] Implement the durable migration state machine and capture boundary in `packages/domain/src/security/migration.ts`, `packages/database/src/repositories/security/migration-repository.ts`, and `packages/database/src/repositories/security/migration-checkpoint-repository.ts` (FR-028, FR-029). *(The pure half: the ordering, the three gates, checkpoint advancement, and the fault-resume rule. The durable half — rows, checkpoints, the backfill itself — is T096 and T097 and will call these predicates rather than restating them.)* (NO CI here)
