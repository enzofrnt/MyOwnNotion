...
- [ ] T006 Add the security command/test inventory and exact local-checks→pull-request→`main` gate script names in `package.json`, `scripts/ci/check-toolchain.ts`, and `docs/development.md`, including unit, property, integration, contract, migration, e2e, build, Compose, and release-gate checks (FR-033, FR-035).
