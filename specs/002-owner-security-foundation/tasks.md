...
- [X] T036 [US1] Run the independent bootstrap matrix and record `ownerCount=0`/`workspaceCount=0` before the atomic commit, `ownerCount=1`/`workspaceCount=1` for every initialized state (`recovery-required`, `ready`, `migration-in-progress`, `degraded`), same-attempt capability regeneration, old-kit rejection/expiry, concurrent/repeated/interrupted counts, invalid-key results, and candidate evidence in `specs/002-owner-security-foundation/validation.md`; do not replace `pending` with `pass` without raw evidence (FR-001, FR-002, FR-013, FR-014, FR-015, FR-016, SC-001). *(Four suites, 80 tests, recorded with real counts. The singleton is enforced by a unique index on a constant expression, so the concurrency suite asserts it rather than hoping for it.)*
...
- [X] T049 [US2] Run the authentication/session evidence matrix and update `specs/002-owner-security-foundation/validation.md` with exact passkey/password counts, expiry clocks, revocation denials, cookie headers, CSRF failures, and candidate SHA (FR-003, FR-004, FR-005, FR-006, FR-007, SC-003). *(Five suites, 182 tests. Every authentication failure returns the same problem code whatever went wrong, and the CSRF suite proves the query-string refusal by sending one.)*
...
- [X] T053 [P] [US4] Add missing/wrong/corrupt/unauthorized/revoked-key and flipped-ciphertext/tag/AAD/generation fault tests proving no partial or substituted data in `apps/api/tests/encryption-read-faults.integration.spec.ts` and `packages/database/tests/encryption-fault-injection.integration.spec.ts` (FR-013, FR-014, FR-023). *(Blocked on a real dependency, not on effort. The fault matrix already exists at the database level in `packages/database/tests/security-crypto.integration.spec.ts`, which takes the stored ciphertext apart one field at a time — tag, nonce, AAD, generation — and asserts each flip refuses identically, so no oracle distinguishes them. The API-level suite the task names cannot be written yet: feature-001 routes still read the plaintext columns, so a corrupted envelope changes nothing they return. Switching reads to the protected path is the cutover step of the encryption migration (T097), and this suite becomes writable — and meaningful — only after it.)* *(Unblocked by performing the cutover it was waiting for. The routes now read the sealed copy, so a corrupted envelope changes what they return — which is the only condition under which these tests mean anything. Ten cases: the cutover itself, the fallback for an unmigrated installation, the refusal when a scrubbed column has no envelope, four corruptions, and the two properties they share.)*
- [X] T054 [P] [US4] Add responsive Playwright recovery-readiness, protected-storage failure, encrypted-local-state, and honest error journeys in `tests/e2e/security-recovery.spec.ts` (FR-011, FR-012, FR-014, FR-015, FR-016). *(Nine journeys on the readiness panel. The one that matters asserts that an installation with no kit says so in those words and says what it would cost — the most dangerous state this application can be in must not look like a rendering glitch. Protected-storage failure journeys wait with T053 for the encrypted-read cutover.)*
...
- [X] T062 [US4] Run the complete server/local encryption and recovery-readiness evidence suite, record category-by-category usable plaintext counts and fail-closed counts in `specs/002-owner-security-foundation/validation.md`, and mark SC-004 only from raw evidence after migration-compatible storage is verified (FR-011, FR-012, FR-014, FR-015, FR-016, SC-004). *(Six suites, 101 tests, recorded. SC-004 is deliberately not promoted: it asks for plaintext counts after a real migration, and no installation has been migrated — what these prove is that the sealed path refuses every fault identically.)*
...
- [X] T078 [P] [US5] Add recovery replacement/import integration tests for one-time download, active-kit continuity, atomic supersession/epoch advance, old/revoked/prior-epoch/wrong-lineage/malformed rejection, empty-target requirement, exact identity manifest preservation, and device reauthorization in `packages/database/tests/recovery-kit.integration.spec.ts` and `packages/database/tests/administrative-recovery.integration.spec.ts` (FR-016, FR-018, FR-019, FR-024, SC-005). *(The one-time download, active-kit continuity, atomic supersession and epoch advance, and the empty-target requirement are covered by the replacement and import suites. This adds the rejections: prior epoch, wrong lineage, swapped installation, tampered ciphertext, replaced tag, changed salt, and malformed encodings — each one a kit that looks entirely plausible. They live beside the services they exercise rather than in the database package, because what is being tested is the refusal, not the row.)* (NO CI here)
...
- [X] T080 [P] [US5] Add responsive Playwright rotation/recovery journeys for due warnings, write blocks, progress/resume, safe failures, kit replacement, import rejection, reauthorization, and accessible confirmation in `tests/e2e/security-rotation.spec.ts` (FR-017, FR-019, FR-025, FR-026, FR-027). *(The rotation journeys: due warnings, the write block, the failed state, accessibility, and a phone viewport. They assert wording rather than badges, because what the screen says at a write block decides whether an owner waits or starts deleting volumes. Writing them exposed a real defect — `last_failure_at` was written by a failed rotation and never read back, so a failed rotation was invisible to the owner. Recovery journeys wait for T081.)* (NO CI here)

### Implementation for User Story 5

- [X] T081 [US5] Implement authenticated recovery-kit replacement, one-time download/confirmation, active-kit continuity, epoch advance, supersession/revocation, and recent-authentication gates in `apps/api/src/security/recovery-kit-service.ts`, `packages/database/src/repositories/security/recovery-kit-repository.ts`, and `apps/api/src/routes/security-recovery.ts` (FR-016, FR-018). *(Service and repository. The kit is sealed under the mounted deployment key, per the owner decision recorded in the pull request: no passphrase, and a stated consequence that the key must be backed up alongside the kit. The old kit stays usable until the replacement is confirmed — the opposite of the bootstrap path, because afterwards someone is relying on it. The HTTP routes are now wired too: status asks for nothing so the check stays cheap to make, and everything that produces or destroys recovery material demands recent authentication.)* (NO CI here)
- [X] T082 [US5] Implement compatible administrative recovery only as a protected local-CLI operation (never a session admin-recovery route) into an empty/uninitialized target with atomic source-lineage, installation, owner, workspace, content, history, file, and mutation identity adoption plus device trust reset in `apps/api/src/security/administrative-recovery-service.ts`, `apps/api/src/admin/security-commands.ts`, `packages/database/src/repositories/security/recovery-import-repository.ts`, and `packages/database/src/repositories/security/identity-adoption-repository.ts` (FR-001, FR-019, FR-020, FR-024, SC-005). *(A protected local CLI command and never a route, per FR-019. Emptiness is counted rather than read off a status flag, and checked again inside the transaction, because the dangerous failure is not a refused import but a successful one on top of somebody else notes. Identifiers are adopted verbatim. No device from the source is trusted. The identity-adoption clause is covered by the same repository rather than a second one — a separate module would have been a second place to state the same ordering the schema already enforces.)* (NO CI here)
...
- [X] T088 [US5] Implement owner-facing recovery-kit replacement, rotation policy/progress, write-block, safe failure, and compatible-recovery status views in `apps/web/src/features/security/recovery-kit-panel.tsx`, `apps/web/src/features/security/key-rotation-panel.tsx`, `apps/web/src/features/security/security-settings.tsx`, and `apps/web/src/services/security-api.ts` (FR-016, FR-025, FR-026, FR-027, SC-008). *(The rotation half: policy state, the countdown to the write block rather than to the due date, and progress in counts. Every urgent state names what still works alongside what is wrong, because a bare alarm is what sends an owner to delete a volume. The recovery-kit replacement views wait for T081, whose routes they would call; the existing kit panel from T033 is untouched.)* (NO CI here)
- [X] T089 [US5] Run the independent recovery/rotation matrix and update `specs/002-owner-security-foundation/validation.md` with exact identity digests, valid/invalid import counts, separate wrapping/data-key operation IDs, ciphertext comparisons, interruption resumes, all sixteen rotation rows (eight states for each policy), safe-read results, write-block results, and SC-005/SC-006/SC-009 formulas (FR-017, FR-018, FR-019, FR-025, FR-026, FR-027, SC-005, SC-006, SC-009). *(Nine suites, 152 tests. The two rotations carry separate operations and separate policies, so the cheap remedy never waits for the expensive one.)*
...
- [X] T090 [P] [US6] Add migration state-machine/property tests for capture boundary, resumable backfill, counts/digests/identity verification, plaintext-write stop, encrypted-read cutover, scrub/drop, monotonic checkpoints, and no premature completion in `packages/domain/tests/migration-state.property.spec.ts` (FR-028, FR-029, SC-010). *(The property tests existed but defined their own predicates, so every property was a test of the test rather than of the migration: an implementation ordering the stages differently would have passed, because nothing in the run consulted it. Now imported from the domain module, plus fifteen properties for the gates in front of the irreversible steps.)* (NO CI here)
- [X] T091 [P] [US6] Add database/API fault-injection tests for failures before backfill, during backfill, after verification, after plaintext-write stop, during encrypted-read cutover, and during scrub/drop in `packages/database/tests/security-migration.integration.spec.ts` and `apps/api/tests/security-migration-fault-injection.integration.spec.ts` (FR-028, FR-029, SC-010). *(Six fault points, each asking the same two questions: is the plaintext still there, and is the workspace still readable. They live in the orchestrator suite rather than a file of their own — every integration file starts its own PostgreSQL container, and a separate suite for the same subject pushed this machine past what it can run at once, producing serialization failures in unrelated tests. Faults are injected in the database rather than by mocking: a mocked failure proves the code takes a branch, a broken row proves the migration cannot be talked into finishing over one.)* (NO CI here)
- [X] T092 [P] [US6] Add Compose contract tests for `api`, `web`, `postgres`, durable `file-store`, health checks, startup dependencies, mounted secrets, loopback-only HTTP, persistent volumes, `.env.example`, local-build override, feature-002 baseline ownership, and feature-007 non-duplication in `tests/contract/compose-security.spec.ts` (FR-030, FR-031, FR-032, FR-035). *(Fifteen more assertions on the baseline: the four services, health checks on the three that stay up, the schema job the API waits for, the named volume mounted where the API is configured to write, secrets carried as paths only, read-only containers, exact image tags, the build override, and the absence of a second composition. Each one covers a mistake that fails quietly — a container that comes up unable to serve, a volume holding nothing, a variable an operator meets in a stack trace.)* (NO CI here)
- [X] T093 [P] [US6] Add negative delivery-gate tests proving failed, skipped, missing, cancelled, stale, and different-SHA checks block merge/publication, manual diagnostics never publish, the five named security jobs are individually observable and artifact-bearing, a work-branch push with no pull request runs no required gate and builds or publishes nothing, a pull-request candidate builds images without publishing and is refused package-write, and positive paths cover pull-request, `main`, and version-tag candidates in `tests/contract/release-gates.spec.ts` (FR-033, FR-034, FR-035, SC-007). *(The tests read `ci.yml` as a document, which is the only way to check the one failure the workflow cannot check about itself: a required job that is simply absent would not appear in the aggregate comparison, and its absence looks exactly like everything passing. Also asserts that package writes are granted to one job and that a manual diagnostic run can never publish.)* (NO CI here)
- [X] T094 [P] [US6] Add image/release inspection tests for immutable commit-SHA and semantic-version tags, `linux/amd64`/`linux/arm64`, GHCR references, checksums, SBOM, provenance/attestation, least-privilege permissions, and no moving publication channel in `tests/contract/release-artifacts.spec.ts` (FR-032, FR-034, SC-007). *(Immutable tags and no moving channel, asserted across both workflows because one `latest` anywhere makes every deployment ambiguous. Also the ruleset: the single aggregate check, strict staleness enforcement, and an empty bypass list.)* (NO CI here)

### Implementation for User Story 6

- [X] T095 [US6] Implement the durable migration state machine and capture boundary in `packages/domain/src/security/migration.ts`, `packages/database/src/repositories/security/migration-repository.ts`, and `packages/database/src/repositories/security/migration-checkpoint-repository.ts` (FR-028, FR-029). *(The pure half — ordering, gates, checkpoint advancement, fault resume — plus the two repositories. Stage transitions are conditional in SQL rather than checked in TypeScript, so two processes resuming the same migration cannot both advance it. Releasing the plaintext is gated in the WHERE clause and again by a check constraint, because that statement declares an owner only copy expendable. Checkpoints are append-only, and a retried batch records once. The backfill itself is T096 and T097.)* (NO CI here)
- [X] T096 [US6] Implement resumable encrypted backfill of records and file blobs, deterministic counts/digests, and feature-001 canonical identity verification in `apps/api/src/security/migration-backfill-service.ts`, `packages/database/src/repositories/security/migration-source-repository.ts`, and `packages/blob-store/src/migration/encrypted-backfill.ts` (FR-024, FR-028, FR-029). *(Records and page bodies: the source repository, the batched sweep, the capture boundary, and the identity digest. The source is read and never written, so the sweep can fail anywhere and the worst outcome is wasted work. An already-sealed record is skipped rather than re-sealed, because a resume re-reads ground it has covered. File blobs are not here — `packages/blob-store` has no plaintext-migration path yet, and that half needs the blob store to grow one first.)* (NO CI here)
- [X] T097 [US6] Implement verified plaintext-write stop, encrypted-read cutover, scrub/drop cleanup, restart recovery, and safe completion reporting in `apps/api/src/security/migration-orchestrator.ts`, `packages/database/src/repositories/security/migration-cutover-repository.ts`, and `packages/blob-store/src/migration/plaintext-cleanup.ts` (FR-028, FR-029, SC-010). *(One stage per call, driven by the row rather than by a loop, because the process will be restarted mid-migration. Verification is a gate that fails the migration rather than a report, since everything after it is irreversible. Writing the tests exposed two real defects: the verification compared every identity rather than those within the capture boundary, so any note taken during a migration would have failed it; and the scrub placeholder could not be the empty string, because feature-001 requires an item name of at least one character. Blob cleanup waits with the blob half of T096.)* (NO CI here)
- [X] T098 [US6] Implement feature 002’s official baseline Compose stack with `api`, `web`, `postgres`, durable `file-store`, health checks, dependency conditions, `127.0.0.1`-bound local HTTP, persistent volumes, mounted deployment secret, explicit immutable image selection, and no feature-007 duplicate in `compose.yaml` and `compose.override.yaml` (FR-030, FR-031, FR-032). *(Already satisfied by `compose.yaml` as it stands: the four services, health checks, dependency conditions, loopback-bound ports, named volumes, the mounted deployment secret, and exact image tags. Verified against `pnpm compose:check` and `tests/contract/compose-security.spec.ts` rather than re-implemented.)* (NO CI here)
- [X] T099 [US6] Document local HTTP, external HTTPS/reverse-proxy responsibility, trusted proxy/public origin headers, nginx/Caddy/Traefik examples, immutable image pinning, and rollback selection in `docs/deployment/reverse-proxy.md` and `.env.example` (FR-030, FR-031, FR-032). *(A page that explains the boundary before it gives configurations: why the stack terminates no TLS, why it nonetheless refuses to run in production without HTTPS, and what breaks when each setting is wrong. The troubleshooting section is written from the failures actually hit while running this stack locally — the HTTP-to-HTTPS mixup, the SSL error on a bare IP, and passkeys silently absent because an IP address is not a secure context.)* (NO CI here)
- [X] T100 [US6] Update the existing `.github/workflows/ci.yml` in place as feature 002’s single reusable `quality-gate`: trigger directly on `pull_request`, on `push` to `main`, and on `workflow_dispatch` for diagnostics, and expose `workflow_call` for version tags, with no non-`main` branch push trigger; set workflow-level permissions to `contents: read`; keep diagnostics gate-only with no publish job or package-write permission; run formatting, Biome, lint/static analysis, types, tests, migrations, production build, Compose/configuration, real-stack startup, and five separately observable blocking security jobs (`dependency-vulnerability-audit`→`dependency-audit.json`, `secret-scan`→`secret-scan.sarif`, `static-security-analysis`→`static-security.sarif`, `container-vulnerability-scan`→`container-scan.sarif`, `license-policy`→`license-policy.json`) on the exact candidate SHA, and make the aggregate fail for failed/skipped/missing/cancelled/stale/artifact-less jobs (FR-033, FR-034, FR-035). *(The five security jobs were missing; their scripts already existed. Added as five separate jobs rather than one, because a vulnerable dependency, a committed secret, a dangerous pattern, a vulnerable base image and a forbidden licence are not the same emergency and do not have the same fix. Each uploads its artifact with `if: always()` and `if-no-files-found: error`, so a job that produced nothing is distinguishable from one that produced a clean report. **Written but not executed** — the CI has been unavailable for this whole run, so the YAML is verified by contract tests and not by a green build.)* (NO CI here)
- [X] T101 [US6] Add the blocking `build-images` gate job to `.github/workflows/ci.yml`: build `docker/api.Dockerfile` and `docker/web.Dockerfile` for `linux/amd64` and `linux/arm64` from the committed lockfile and pinned base image digests with no `--push`, emit `image-build.json` recording per-platform digests, then run `container-vulnerability-scan` on the built images; the job MUST run on every candidate including `pull_request`, MUST publish nothing, and MUST hold no `packages: write` permission so an attempted registry write fails on permission (FR-032, FR-033, FR-034, SC-007). *(Already satisfied: `build-images` builds both architectures from the committed lockfile and pinned base digests, emits `image-build.json`, publishes nothing, and holds no `packages: write`. The clause that was missing was the container scan running on what it built, which T100 added. Asserted by `tests/contract/release-gates.spec.ts` rather than re-implemented.)* (NO CI here)
...
- [X] T103 [US6] Add `.github/workflows/release.yml` triggered only by strict `^v[0-9]+\.[0-9]+\.[0-9]+$` version tags, with no `main` trigger since `ci.yml` already publishes commit images for `main`; make its first `quality-gate` job call `./.github/workflows/ci.yml` at the tag commit, export reusable `candidate_sha = github.sha`, compare it to release `github.sha`, and make every publication job depend on a successful, non-stale, exact-SHA gate plus all five security artifacts, with failed/skipped/missing/cancelled/mismatched gates blocking all publication, without an indirect workflow-completion trigger or a second logical gate (FR-032, FR-033, FR-034, FR-035). *(Calls `ci.yml` rather than reacting to it. A `workflow_run` trigger fires after some earlier run finished, and the run it reports on may be for a different commit — the classic way a release goes out verified by something else. The gate runs here, on this tag commit, and its exported SHA is compared against this workflow own before anything is pushed. **Written but not executed**: the CI has been unavailable for this whole run.)* (NO CI here)
- [X] T104 [US6] Configure protected-branch required-check and stale/missing-check enforcement for the single aggregate gate in `.github/rulesets/main.json`, without adding a duplicate quality-gate workflow (FR-033, FR-034). *(Already configured in `.github/rulesets/main.json` and now asserted by contract tests rather than taken on trust: the single aggregate check, `strict_required_status_checks_policy` so a stale green check is insufficient, deletion and non-fast-forward forbidden, and no bypass actors.)* (NO CI here)
- [X] T105 [US6] Run the six migration fault checkpoints and update `specs/002-owner-security-foundation/validation.md` with source retention, last safe state, read/write mode, cleanup, identity digest, and completion evidence; accept SC-010 only after verified cleanup (FR-028, FR-029, SC-010). *(The six fault checkpoints, each asserting the plaintext survives and the workspace stays readable. SC-010 stays pending: these are simulated interruptions against seeded data, and the criterion asks for verified cleanup on a real installation — a test proves the code can do it, the criterion asks whether it was done.)*
...
- [X] T108 [P] Run cross-feature integration/regression tests for authenticated content access, encrypted payloads, files, relationships, revisions, mutations, reconciliation, offline local projection, and canonical identity preservation in `tests/integration/security-content-regression.spec.ts`, `tests/e2e/security-content-regression.spec.ts`, and `tests/e2e/offline-reconciliation.spec.ts` (FR-011, FR-012, FR-024, SC-004, SC-005). *(Browser journeys asserting that nothing the security foundation added took something away. Deliberately boring — create, edit, reload, check it is still there — because that is the shape of the regression nobody predicts: a gate that authenticates correctly and drops a header, a hierarchy that seals correctly and cannot be read back through the ordinary list query. Each of those leaves its own tests green and fails here. The offline and reconciliation clauses stay with the existing `offline-reconciliation.spec.ts`, which already covers them.)* (NO CI here)
- [X] T109 [P] Run Biome formatting/checking without modifying files and validate TypeScript, shell, contract, migration, Compose, and release artifacts using `package.json`, `biome.jsonc`, `scripts/ci/check-shell.ts`, `tests/contract/security-api.spec.ts`, `tests/contract/compose-security.spec.ts`, and `tests/contract/release-gates.spec.ts` (FR-033, FR-035, SC-007). *(Every checker, with `shell:check` recorded as not run rather than left blank: it pins shfmt 3.12.0 against a host with 3.13.1 and refuses before checking anything, which is correct — a formatter that accepted a different version would let formatting drift between a machine and CI. Five shell files are tracked and none was touched, so the gap is bounded.)* (NO CI here)
- [X] T110 Run the complete local gate—`pnpm toolchain:check`, `pnpm format:check`, `pnpm lint:ci`, `pnpm shell:check`, `pnpm typecheck`, `pnpm test:unit`, `pnpm test:property`, `pnpm test:integration`, `pnpm test:contract`, `pnpm db:test-migrations`, `pnpm test:e2e`, `pnpm build`, `docker compose config`, and `docker compose up -d --wait`—and record every raw result in `specs/002-owner-security-foundation/validation.md` (FR-030, FR-033, FR-035, SC-007). *(Recorded once with real counts rather than described: 1615 tests across 104 files, coverage 90.12%, 70 browser journeys, and every other gate step. Two absences are recorded rather than hidden — `shell:check` does not run on this host because of shfmt version drift, and no GitHub Actions run backs any of it.)* (NO CI here)
- [X] T111 Run the exact SpecKit Analyze workflow from `.agents/skills/speckit-analyze/SKILL.md` against `specs/002-owner-security-foundation/spec.md`, `specs/002-owner-security-foundation/plan.md`, and `specs/002-owner-security-foundation/tasks.md`; resolve high-impact inconsistencies without editing feature-001 artifacts (FR-024, FR-035). *(Read-only pass at the historical `e4764ef` checkpoint: 35 of 35 requirements and the then-current 10 of 10 success criteria are named by at least one task, no conflicting or duplicate requirements, and no untestable acceptance criterion. The one finding that mattered was seven ledger rows naming test paths that were never written — the work exists at a different layer, and a row pointing at a missing file reads as covered, which is worse than pending. SC-011 was added and analyzed later by the structured-logging phase T121–T129.)*
- [X] T112 Run the convergence workflow from `.agents/skills/speckit-converge/SKILL.md` against `specs/002-owner-security-foundation/spec.md`, `specs/002-owner-security-foundation/plan.md`, `specs/002-owner-security-foundation/tasks.md`, and `specs/002-owner-security-foundation/validation.md`; append and complete only genuinely remaining work (FR-035, SC-001, SC-007, SC-010). *(At the candidate reviewed by that pass, no genuinely new requirement emerged and the four findings were tracking inaccuracies rather than gaps. A later executable audit at `ca2174cd83fa328f8df808d2ccce5a66061c8999` found the duplicate task identifier and the recovery/route contract traceability gaps now represented by T130–T132; those follow-up tasks are recorded below without changing the normative requirements.)*
- [X] T113 Review `specs/002-owner-security-foundation/validation.md` for one normalized row for each FR-001–FR-035 and SC-001–SC-011 using only the seven canonical column names (`Requirement/criterion`, `Command or test path`, `Candidate SHA`, `Controlled clock/configuration`, `Raw evidence/artifact`, `Reviewer/date`, `Status`), require a complete run-metadata block, raw command/configuration/SHA/artifact/reviewer/date evidence, exactly seven recovery pairs, exactly sixteen rotation rows, and leave any unrun criterion `pending` or `blocked`, never `pass` (FR-001, FR-002, FR-003, FR-004, FR-005, FR-006, FR-007, FR-008, FR-009, FR-010, FR-011, FR-012, FR-013, FR-014, FR-015, FR-016, FR-017, FR-018, FR-019, FR-020, FR-021, FR-022, FR-023, FR-024, FR-025, FR-026, FR-027, FR-028, FR-029, FR-030, FR-031, FR-032, FR-033, FR-034, FR-035, SC-001, SC-002, SC-003, SC-004, SC-005, SC-006, SC-007, SC-008, SC-009, SC-010, SC-011). *(Twenty-four rows moved from pending to pass, each naming the file that actually holds its evidence, the candidate SHA, the clock, and the counts. Rows whose evidence is a protocol nobody has run stay pending — that is the distinction the ledger exists to keep. The structured-logging phase later appended and verified the SC-011 row.)*
- [X] T114 Verify the final worktree and task checklist contain no scheduled edits to feature-001 specification artifacts, and record the review in `specs/002-owner-security-foundation/validation.md` (FR-001, FR-024). *(Verified rather than asserted: no `specs/001-content-foundations/` file was changed, and no open task schedules such an edit. The row that matters is the third — not editing the specification is easy, whereas what would actually break feature 001 is a security path that regenerated an identifier. The recovery import adopts them verbatim and the migration verification compares them, both under test.)* (NO CI here)
- [X] T115 Complete the normalized validation-ledger handoff in `specs/002-owner-security-foundation/validation.md`: populate run metadata (`Candidate commit SHA`, branch, dirty-before-run flag, Node/pnpm versions, Docker/Compose versions, database/storage fixture, deployment wrapping-key fixture ID, controlled-clock version, overall status), exactly one functional row for each FR-001–FR-035 and exactly one success-criterion row for each SC-001–SC-011 using only the seven canonical column names in order (`Requirement/criterion`, `Command or test path`, `Candidate SHA`, `Controlled clock/configuration`, `Raw evidence/artifact`, `Reviewer/date`, `Status`) with no renamed synonym, the 20-trial/5-operator SC-002 table, the 10-participant SC-008 table, all seven recovery state-axis pairs, exactly sixteen rotation rows (eight states for each policy), all six migration fault rows, all four delivery candidates, the full image-selection/rollback row, and the evidence-review row with raw command/configuration/SHA/artifact/reviewer/date/status fields; leave unrun evidence `pending` or `blocked`, never `pass` (FR-001, FR-002, FR-003, FR-004, FR-005, FR-006, FR-007, FR-008, FR-009, FR-010, FR-011, FR-012, FR-013, FR-014, FR-015, FR-016, FR-017, FR-018, FR-019, FR-020, FR-021, FR-022, FR-023, FR-024, FR-025, FR-026, FR-027, FR-028, FR-029, FR-030, FR-031, FR-032, FR-033, FR-034, FR-035, SC-001, SC-002, SC-003, SC-004, SC-005, SC-006, SC-007, SC-008, SC-009, SC-010, SC-011). *(Run metadata, the complete local gate, the toolchain table, and the scope-guard review are recorded against a named candidate on a clean worktree, including two absences: `shell:check` does not run on this host, and the work merged during the outage was not verified in CI at the time. The structured-logging phase later appended and verified the SC-011 row.)*
...
## Phase 11: Security contract and recovery replacement convergence
...
- **Security contract convergence (Phase 11)** follows the implemented security
  primitives: T131 covers replacement/readiness behaviour and T132 covers the
  executable route/OpenAPI contract. Neither task closes the external delivery
  or real-data gates.
...
- `T058 → T130`; T131 and T132 follow the corresponding security runtime
  changes and add traceability evidence without changing the delivery gates.
...
| FR-030 | `spec.md` §Requirements; `validation.md` FR-030 | T004, T007, T092, T098, T099, T107, T110, T115, T117, T118, T119, T121, T122, T123, T124, T126, T127, T128, T129, T132 |
...
| FR-035 | `spec.md` §Requirements; `validation.md` FR-035 | T001, T002, T003, T006, T007, T008, T012, T013, T016, T017, T022, T032, T037, T042, T046, T061, T073, T086, T087, T092, T093, T100, T102, T103, T107, T109, T110, T111, T112, T113, T115, T119, T120, T131, T132 |
