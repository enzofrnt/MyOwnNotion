...
- [x] Bootstrap has one unambiguous atomic transition: every pre-confirmation state remains `0/0` with no committed owner/workspace rows; successful passkey verification plus password recording plus confirmation commits the sole owner with both credentials, binds the feature-001 canonical workspace, and changes the installation to ready with `1/1`, without requiring recovery-kit download.
...
- [x] Roadmap ownership assigns the FR-030–FR-035 baseline secure delivery
  foundation to feature 002, backup/recovery/update safeguards to feature 007,
  and protected private search indexes to feature 008.
...
- FR-016 covers post-bootstrap recovery-kit regeneration and replacement only: provisional deliveries may be safely regenerated; after an active confirmed kit exists, replacement requires recent authentication and advances the epoch atomically; expiry never affects an active confirmed kit. Bootstrap itself does not gate on kit download.
...
- The roadmap assigns the baseline secure
  Compose/env/reverse-proxy/CI/GHCR/release foundation to feature 002,
  backup/recovery/update safeguards to feature 007, and protected private
  search indexes to feature 008; feature-001 T105 and T106 are recorded as
  complete in the baseline delivery work.
