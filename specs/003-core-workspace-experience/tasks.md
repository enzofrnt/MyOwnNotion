...
- [X] T061 [US4] Make the workspace layout responsive from 320 pixels in `apps/web/src/global.css` and `apps/web/src/app.tsx` (FR-021)
...
- [X] T063 [P] [US4] Contain wide content — code blocks, long links — inside their own scroll containers in `apps/web/src/global.css` so the page never widens
- [X] T064 [P] [US4] Ensure touch targets are at least 44×44 pixels across the editor and tree controls in `apps/web/src/global.css`
