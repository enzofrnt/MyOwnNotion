...
## Phase 11: Atomicité page-state et replay historique

- [x] T115 [P] Ajouter et exécuter les régressions d'atomicité de projection page-state, de validation des arbres de blocs, de replay causal des branches hors ligne historiques et d'archivage des opérations, en couvrant les rejets, les réexécutions idempotentes et l'absence d'état partiellement appliqué dans `packages/client-core/tests/apply-to-projection.spec.ts`, `packages/page-state/tests/block-tree.edge-cases.spec.ts`, `packages/page-state/tests/legacy-offline-branch.property.spec.ts`, `apps/api/tests/page-operation-archive.spec.ts` et `apps/api/src/page-state/` per FR-026, FR-027, FR-035 et US3/US4. Preuve d'implémentation et de tests : commit `ca2174cd83fa328f8df808d2ccce5a66061c8999`; T114 reste ouvert pour la CI, la fusion, `main` et le HAR réel.
