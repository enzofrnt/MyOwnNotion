...
- [ ] T007 Add purpose-specific content-index key derivation in `apps/api/src/security/key-hierarchy.ts` and encrypted file metadata/manifest entities in `apps/api/src/security/protected-content.ts`; verify no recovery-key accessor is reused (FR-002/FR-006).
- [ ] T008 Add transaction-aware content/upload chunk repositories and candidate lookup in `packages/database/src/repositories/`, with format-specific reads and generation-lock boundaries (FR-003/FR-005/FR-006).
...
- [ ] T014 [US1] Implement authenticated streaming full/single-range downloads in `apps/api/src/routes/files.ts` and `apps/api/src/files/file-range.ts`; verify headers, 206/416, corruption and no unauthenticated bytes (FR-003/FR-004/FR-008).
