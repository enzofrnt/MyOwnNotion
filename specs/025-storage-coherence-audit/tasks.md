...
- [x] T057 Align the multipart browser regression with T055's bounded retry contract after the complete WebKit gate records two genuine 409 responses followed by 201. Verify every retry response/code, two-to-three total attempts, one stable mutation identity and one canonical hierarchy entry in `tests/e2e/files.spec.ts`; retain unrelated-error refusal and renewed complete gates (FR-004/FR-005/FR-008/FR-010/FR-013, audit A36).
...
- [x] T060 [US4] Reproduce WebKit navigation canceling a successful change-feed response during body consumption; convert a 2xx body-read rejection into a bounded offline result without replaying an uncertain request, and align the durable page-link journey with the workspace synchronization barrier before reload. Add focused unit/browser evidence and retain exact delivery gates (FR-011/FR-012/FR-013, audit A45).
- [x] T061 [US1] Register each protected ciphertext key in the existing bounded garbage ledger before physical publication, acknowledge it only after canonical chunk/manifest persistence, and make cleanup reclaim rollback/crash leftovers while preserving any canonical reference. Reproduce late SQL, publication and physical-write failures in `apps/api/tests/protected-file-service.integration.spec.ts`; run focused tests, types and Biome (FR-004/FR-005/FR-008, audit A46).
- [x] T062 [US1] Reproduce the renewed exact Bun 1.4.2 gate's 268.9 MiB SC-004 overrun, keep positional reads on direct `Buffer` storage while returning the public `Uint8Array` view, and preserve complete-fill, EOF, digest, empty-file and output-ownership guarantees. Repeat the unchanged 2 GiB fixture three times under both maintained `--smol` and ordinary Bun modes without changing the 256 MiB budget, sampling or GC behavior (FR-004/FR-008, SC-004, audit A47).
- [x] T063 [US1] Saturate all ten primary PostgreSQL pool slots around a protected ingest, reproduce the write-intent request blocking on an eleventh connection, and reserve one independently closed database lane for the bounded journal. Prove publication completes while the canonical transaction owns the final primary slot, retain serialized journal writes, and verify service, file, lifecycle, types and Biome checks (FR-004/FR-005/FR-008, audit A48).

## Phase 13: Convergence — protected placeholder reservation

- [x] T064 [US4] Reserve the exact trimmed U+FFFD display name used to neutralize protected storage; reproduce and reject page creation/rename, direct and resumable file publication, MCP writes, retained-revision restoration and Notion/Obsidian preview before canonical mutation, while preserving longer authored names that contain the character, internal scrub writes, and explicitly inventoried V0 titles/filenames during the authenticated historical transition. Delete a newly published legacy envelope before global verification and prove the transition remains before verified cutover. Record RED/GREEN, type and formatting evidence (FR-007/FR-011/FR-014, SC-003/SC-007, audit A49).

## Phase 14: Convergence — protected structured-payload reservation

- [x] T065 [US4] Centralize and reserve only the exact `{"$myownnotionProtected":1}` structured storage marker at page-document and relationship write boundaries; reject creation, replacement and retained-history restoration before mutation, preserve authored objects with additional keys, and migrate explicitly inventoried V0 page bodies, retained snapshots and relationship metadata through authenticated checkpoints. Require their protected copy for post-publication digest, global verification and retirement; record RED/GREEN, type and formatting evidence (FR-007/FR-011/FR-014, SC-003/SC-007, audit A50).

## Phase 15: Convergence — migration, archive and history hardening

- [x] T066 [US2] Revalidate every authenticated protected envelope at `finishVerification`, `cutover` and final `retireNext` completion; delete or alter a published envelope between lifecycle boundaries and prove the transition remains blocked until the exact copy is restored (FR-007/FR-014, SC-003, audit A51).
- [x] T067 [US2] Authenticate V0 source provenance from the full backup, receipt, manifest and installation identity, require absence of migration `0006_installation_application_version`, and persist source identity/provenance in the version-2 inventory across resume and re-inventory (FR-007/FR-014, SC-003, audit A52).
- [x] T068 [US2] Preserve legacy-compatible V1 portable archives while enforcing V2 marker and exact trimmed-name reservations; validate every canonical version and page-operation payload before any archive byte is emitted or any restore-target mutation begins, verify strict TAR framing and the streaming producer, and exercise a real V1 database restore (FR-007/FR-014, SC-003, audit A53).
- [x] T069 [US4] Make protected history fail closed: return 500 for a missing envelope and 410 only for expiry on GET/restore, remove raw fallback from compaction and legacy-branch conversion, and require an explicit client base with bounded expiry for the supported legacy branch path (FR-014, SC-007, audit A54).
- [x] T070 [US4] Require the exact protected-marker matcher to inspect `Reflect.ownKeys`, including symbol and non-enumerable keys, and accept only one own string key with the expected value while preserving multi-key authored objects (FR-014, SC-003/SC-007, audit A55).
...
- [x] T075 [US2] Reject every current, retained or candidate checkpoint whose `throughPageSequence` is beyond the complete update-log head (FR-007/FR-014, SC-002/SC-003, audit A60).
- [x] T076 [US2] Enforce causal timestamp order for checkpoint verification, update compaction, ambiguity resolution, legacy conversion and page bootstrap before any archive byte is emitted or any restore-target mutation begins (FR-007/FR-014, SC-002/SC-003, audit A61).
- [x] T077 [US2] Bind checkpoint frontier to the exact update sequence it covers: sequence zero equals the first update base when present, and sequence N equals update N's retained result frontier. Prove the snapshot opens at that frontier and restore refuses a checkpoint whose content is ahead of its declared sequence (FR-007/FR-014, SC-002/SC-003, audit A62).
- [x] T078 [US2] Require `legacy` operational state to remain empty of checkpoints, updates, frontiers, digests, windows, device receipts, ambiguities and conversions, matching the producer lifecycle and refusing contradictory restoration (FR-007/FR-014, SC-002/SC-003, audit A63).
- [x] T079 [US4] Preserve reusable database source and entry items with zero hierarchy placements after all display hosts are purged while retaining ordinary active-page placement cardinality and canonical database membership/view invariants; verify the focused purge/restore export regression and record the complete-gate failure and correction (FR-011/FR-014, SC-007, audit A64).

## Phase 17: Final independent audit closure

- [x] T080 [US4] Preserve the unchanged absolute coverage budget while integrating the final audit corrections; replace branch-heavy export normalization with the production JSON serializer in `packages/domain/src/export/canonical-export.ts` and `apps/api/src/routes/export.ts`, cover it in `packages/domain/tests/canonical-export.spec.ts`, and run complete coverage on the exact code commit (FR-011/FR-013, SC-007, audit A65).
- [x] T081 [US4] Validate table-column insert/delete commands before mutating the operational block tree in `packages/page-state/src/block-tree.ts` and prove invalid indices, duplicate identities and failed placement edits leave the original page state unchanged in `packages/page-state/tests/block-tree.edge-cases.spec.ts` and `packages/client-core/tests/apply-to-projection.spec.ts` (FR-011/FR-014, SC-007, audit A66).
- [x] T082 [US3] Make authenticated recovery-kit replacement atomic across epoch allocation, one-time download expiry and confirmation in `apps/api/src/security/recovery-kit-service.ts` and `packages/database/src/repositories/security/recovery-kit-repository.ts`; align the API/UI in `apps/api/src/routes/security-recovery.ts`, `apps/web/src/features/security/recovery-replacement-panel.tsx` and cover the ceremony in `apps/api/tests/recovery-kit-replacement.integration.spec.ts`, `apps/web/tests/recovery-replacement.spec.tsx` and `tests/e2e/security-recovery.spec.ts` (FR-009/FR-011/FR-013, SC-005/SC-007, audit A67).
- [x] T083 [US4] Preserve causal and idempotent legacy offline replay for partial text insertion/deletion and invalid placement operations in `packages/page-state/src/legacy-offline-branch.ts`; add generated and edge-case regressions in `packages/page-state/tests/legacy-offline-branch.property.spec.ts` and `packages/client-core/tests/apply-to-projection.spec.ts` without resetting authored local state (FR-011/FR-012/FR-014, SC-007, audit A68).
- [x] T084 [US4] Apply readiness policy by exact HTTP method and declared path in `apps/api/src/security/private-route-guard.ts` and `apps/api/src/app.ts` so recovery/rotation mutations cannot inherit diagnostic access and unknown descendants remain protected; cover the matrix in `apps/api/tests/request-guards.spec.ts` and `tests/e2e/security-recovery.spec.ts` (FR-011/FR-013, SC-007, audit A69).
- [x] T085 [US3] Reconcile device, passkey, recovery, rotation and security-audit runtime contracts in `packages/contracts/src/security-api.ts`, `apps/api/src/routes/devices.ts`, `apps/api/src/routes/security-audit.ts` and `apps/api/src/routes/security-rotation.ts`; scope audit filtering before its limit, serialize concurrent rotations, map stable problem responses, mark authenticated status responses non-cacheable and cover parity in `tests/contract/security-api.spec.ts` plus the API integration suites (FR-009/FR-011/FR-013, SC-005/SC-007, audit A70).
- [x] T086 [US4] Serve only an exact V2 canonical export through `packages/domain/src/export/canonical-export.ts`, `packages/contracts/src/content-api.ts` and `apps/api/src/routes/export.ts`; preserve intentionally open database definition/value objects, bind the delivered manifest to its SHA-256 digest, refuse legacy/corrupt artifacts, resume pending jobs, finalize concurrent recovery once with private no-store caching, and cover the boundary in `apps/api/tests/export.contract.spec.ts` and `tests/contract/export.spec.ts` (FR-005/FR-011/FR-014, SC-002/SC-007, audit A71).
- [x] T087 [US2] Publish and verify the authenticated full-backup HTTP/OpenAPI surface in `apps/api/src/routes/backups.ts` and `specs/024-full-server-backups/contracts/backup-api.openapi.yaml`, with exact success/problem schemas and stable status/rehearsal behavior covered by `apps/api/tests/backup-routes.spec.ts` and `tests/contract/backup-api.spec.ts` (FR-007/FR-011/FR-013, SC-003/SC-007, audit A72).
- [x] T088 [US2] Strengthen archive locks and database/page-operation atomicity proofs for rejected host, parent, placement, table-column and retained operation states in `apps/api/tests/archive-format.spec.ts`, `packages/database/tests/database.integration.spec.ts`, `apps/api/tests/page-operation-archive.spec.ts` and the page-state edge suites; keep every failed command free of partial canonical changes (FR-007/FR-011/FR-014, SC-002/SC-003/SC-007, audit A73).
