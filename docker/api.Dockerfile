# syntax=docker/dockerfile:1.9
#
# API image (feature 002, FR-032/FR-034).
#
# Reproducibility rules:
#   - the base image is pinned by manifest-list digest through BUN_BASE, whose
#     value comes from docker/base-images.json; `bun run images:build` refuses to
#     build with an unpinned base;
#   - dependencies install from the committed bun.lock with `bun ci`; no
#     network resolution may change the tree;
#   - build inputs are non-secret only. No secret value, key file, or `.env` is
#     copied into any layer. The deployment wrapping key is mounted at runtime.
#   - builds for linux/amd64 and linux/arm64 from the same definition.

ARG BUN_BASE
ARG APPLICATION_VERSION=0.1.0

FROM --platform=$BUILDPLATFORM ${BUN_BASE} AS builder
WORKDIR /app
ENV CI=1

# Manifests and lockfile first so dependency layers cache independently of source.
COPY bun.lock bunfig.toml package.json ./
COPY apps/api/package.json apps/api/
COPY apps/web/package.json apps/web/
COPY packages/blob-store/package.json packages/blob-store/
COPY packages/client-core/package.json packages/client-core/
COPY packages/contracts/package.json packages/contracts/
COPY packages/database/package.json packages/database/
COPY packages/domain/package.json packages/domain/
COPY packages/graph/package.json packages/graph/
COPY packages/page-state/package.json packages/page-state/
COPY packages/test-utils/package.json packages/test-utils/
COPY scripts/ci/check-toolchain.ts scripts/ci/

RUN --mount=type=cache,id=bun-install,target=/root/.bun/install/cache \
    bun ci

COPY tsconfig.base.json tsconfig.json ./
COPY packages/ packages/
COPY apps/api/ apps/api/

RUN bun run --filter @myownnotion/api build


FROM ${BUN_BASE} AS runtime
ARG APPLICATION_VERSION=0.1.0
WORKDIR /app
ENV NODE_ENV=production \
    TZ=UTC \
    MYOWNNOTION_APPLICATION_VERSION=${APPLICATION_VERSION} \
    MYOWNNOTION_API_HOST=0.0.0.0 \
    MYOWNNOTION_API_PORT=3001

# Apply the distribution's security point releases. The runtime contains the
# exact Bun binary and the self-contained production bundles, but no workspace
# dependency tree or legacy Node package manager.
RUN apt-get update \
 && apt-get upgrade -y --no-install-recommends \
 && rm -rf /var/lib/apt/lists/*

# Durable blob volume and mounted-secret directory are provided by Compose.
RUN mkdir -p /var/lib/myownnotion/blobs /var/lib/myownnotion/backups \
 && chown -R bun:bun /var/lib/myownnotion

# `dist/` holds both entrypoints: `server.js` and the `migrate.js` the
# Compose `migrate` job runs. The reviewed SQL sits at /app/migrations, which
# is where dist/migrate.js resolves it from.
COPY --from=builder --chown=bun:bun /app/apps/api/dist ./dist
COPY --from=builder --chown=bun:bun /app/packages/database/migrations ./migrations

USER bun
EXPOSE 3001

HEALTHCHECK --interval=10s --timeout=5s --start-period=30s --retries=12 \
  CMD bun --eval "fetch('http://127.0.0.1:'+(process.env.MYOWNNOTION_API_PORT||3001)+'/health').then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))"

CMD ["bun", "dist/server.js"]
