...
describe("WorkspaceSearchIndex properties", () => {
  it("keeps identities unique, rejects stale resurrection and preserves stable order", () => {
    fc.assert(
      fc.property(
        fc.array(
          fc.record({
        ...
        (operations) => {
          ...
          operations.forEach((operation, offset) => {
            ...
            const first = index.search(query("token"), { limit: 50 });
            const second = index.search(query("token"), { limit: 50 });
